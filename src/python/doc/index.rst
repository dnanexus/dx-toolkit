.. DNAnexus Python Bindings documentation master file, created by
   sphinx-quickstart on Mon Feb 27 10:14:25 2012.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

DNAnexus Python Bindings
========================

The DNAnexus :mod:`dxpy` Python library provides Python bindings to
interact with the DNAnexus platform via its API.  Before you start
digging in, you may want to take a look at the `Introduction to the
DNAnexus Platform
<http://wiki.dev.dnanexus.com/Introduction-to-the-DNAnexus-Platform>`_.
The following references may also be useful to you as you develop your
own scripts and apps.

`API Specification <http://wiki.dev.dnanexus.com/API-Specification-v1.0.0/Introduction>`_
    complete details on the API, the lifecycle of different data
    objects, project permissions, etc.
`Execution Environment Reference <http://wiki.dev.dnanexus.com/Execution-Environment-Reference>`_
    how applets and apps are run on the cloud
`Developer Portal <http://wiki.dev.dnanexus.com/Developer-Portal>`_
    links to all other docs, e.g. useful command-line tools

Table of Contents
-----------------

.. toctree::
   :numbered:
   :maxdepth: 9

   dxpy
   dxpy_bindings_intro
   dxpy_api
   dxpy_exceptions

Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

