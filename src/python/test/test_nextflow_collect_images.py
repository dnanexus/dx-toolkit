...
class TestParseDockerRef(unittest.TestCase):
    """Unit tests for _parse_docker_ref(), migrated from nextaur
    DockerImageTokensTest.groovy and extended with additional cases."""

    # ── Cases migrated from nextaur DockerImageTokensTest ──────────────
    ...
    def test_nextaur_parity(self, ref, exp_repo, exp_image, exp_tag, exp_digest):
...
class TestCollectDockerImages(unittest.TestCase):
    ...
    def test_basic_inspect_output(self, mock_run, mock_populate):
...
class TestPopulateCachedFileIds(unittest.TestCase):
    ...
    def test_dedup_by_repository(self, mock_config, mock_find):
        ...
        NOT share cache lookups (bug #8 regression test)."""
        mock_config.get.return_value = "project-123"
        mock_find.return_value = iter([])
        refs = [
