...
class TestDXBashHelpers(DXTestCase):
    ...
    def test_basic(self):
        with temporary_project(
            "TestDXBashHelpers.test_app1 temporary project"
        ) as dxproj:
            ...
            # local checkout
            applet_id = build_app_with_bash_helpers(
                os.path.join(TEST_APPS, "basic"), dxproj.get_id()
            )
    ...
    def test_mount_basic(self):
        with temporary_project(
            ...
            # local checkout
            applet_id = build_app_with_bash_helpers(
                os.path.join(TEST_MOUNT_APPS, "basic"), dxproj.get_id()
            )
    ...
    def test_sub_jobs(self):
        ...
        with temporary_project(
            ...
            def check_file_content(
                out_param_name, out_filename, tmp_fname, str_content
            ):
                """
                ...
                with open(tmp_fname, "r") as fh:
                    ...
                    if not (strip_white_space(data) == strip_white_space(str_content)):
                        raise Exception(
                            "contents of file {} do not match".format(out_param_name)
                        )
                silent_file_remove(tmp_fname)

            check_file_content(
                "first_file", "first_file.txt", "f1.txt", "contents of first_file"
            )
    ...
    def test_parseq(self):
        ...
        with temporary_project(
            ...
            # local checkout
            applet_id = build_app_with_bash_helpers(
                os.path.join(TEST_APPS, "parseq"), dxproj.get_id()
            )
    ...
    def test_file_optional(self):
        ...
        with temporary_project(
            ...
            # local checkout
            applet_id = build_app_with_bash_helpers(
                os.path.join(TEST_APPS, "file_optional"), dxproj.get_id()
            )
    ...
    def test_deepdirs(self):
        ...
        def verify_files_in_dir(path, expected_filenames, dxproj):
            ...
            for elem in dir_listing["objects"]:
                ...
                if handler.name not in expected_filenames:
                    raise Exception(
                        "Error: file {} should reside in directory {}".format(
                            handler.name, path
                        )
                    )
        ...
        with temporary_project(
            ...
            # local checkout
            applet_id = build_app_with_bash_helpers(
                os.path.join(TEST_APPS, "deepdirs"), dxproj.get_id()
            )
            ...
            verify_files_in_dir(
                "/", ["A.txt", "B.txt", "C.txt", "num_chrom.txt"], dxproj
            )


@unittest.skipUnless(
    testutil.TEST_RUN_JOBS and testutil.TEST_BENCHMARKS,
    ...
    def run_applet_with_flags(self, flag_list, num_files, file_size_bytes):
        with temporary_project(
            ...
            remote_file = dxpy.upload_local_file(
                filename="A.txt", project=dxproj.get_id(), folder="/"
            )
...
class TestDXJobutilAddOutput(DXTestCase):
    ...
    test_cases = (
        [
            ...
            ["'32 tables'", "32 tables"],
            ['\'{"foo": "bar"}\'', {"foo": "bar"}],
            [dummy_job_id + ":foo", {"job": dummy_job_id, "field": "foo"}],
            [
                dummy_analysis_id + ":bar",
                {"$dnanexus_link": {"analysis": dummy_analysis_id, "field": "bar"}},
            ],
        ]
        ...
        with tempfile.NamedTemporaryFile(mode="w+") as f:
            ...
            for i, tc in enumerate(error_test_cases):
                with self.assertSubprocessFailure(
                    stderr_regexp="Value could not be parsed", exit_code=3
                ):
                    run(cmd_prefix + " ".join([str(i), "--class " + tc[0], tc[1]]))
...
class TestDXJobutilNewJob(DXTestCase):
    @classmethod
    def setUpClass(cls):
        with testutil.temporary_project(
            name="dx-jobutil-new-job test project", cleanup=False
        ) as p:
            cls.aux_project = p
    ...
    def assertNewJobInputHash(self, cmd_snippet, arguments_hash):
        ...
        env = override_environment(
            DX_JOB_ID="job-000000000000000000000001", DX_WORKSPACE_ID=self.project
        )
    ...
    def test_input(self):
        ...
        third_record = dxpy.new_dxrecord(
            name="third_record", project=self.aux_project.get_id()
        )
    ...
    def test_job_arguments(self):
        test_arguments = (
            ...
            (
                ...
                {
                    "systemRequirements": {
                        "entrypointname": {"instanceType": "foo_bar_baz"}
                    }
                },
            ),
