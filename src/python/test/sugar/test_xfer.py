...
from dxpy.sugar.proc import run
...
class TestUpload(unittest.TestCase):
    ...
    def _assert_tar_equal(self, handler, filenames):
        with isolated_dir():
            ...
            run("tar -xzf {}".format(local_name), block=True)
            run("rm {}".format(local_name), block=True)

            untarred_files = os.listdir(".")
            self.assertEqual(2, len(untarred_files))
    ...
    def test_simple_upload_file(self):
        with isolated_dir():
            ...
            remote_path = os.path.join(self.folder, remote_filename)
    ...
    def test_archive_and_upload(self):
        with isolated_dir():
            ...
            prefix = os.path.join(self.folder, random_name())
            remote_filename = "{}.tar.gz".format(prefix)
...
class TestDownload(unittest.TestCase):
    ...
    def _upload_tar_file(self):
        with isolated_dir():
            filenames = make_random_files(2)
            run("tar cf test.tar.gz {}".format(" ".join(filenames)))
            return filenames, self._upload_file("test.tar.gz")
    ...
    def download_and_decompress_file(self):
    ...
    def download_and_unpack_archive(self):
        filenames, handler = self._upload_tar_file()
        with isolated_dir():
            xfer.download_and_unpack_archive(handler)
    ...
    def test_downloader(self):
        ...
        with isolated_dir():
            ...
            for i, fname in enumerate(tar_filenames):
                path = os.path.join(result["tar"], fname)
