...
def collect_docker_images(resources_dir, profile, nextflow_pipeline_params):
    ...
    Uses the native ``nextflow inspect`` command (NF >= 25.04) instead of
    ``nextflow plugin nextaur:collect docker``.  ``nextflow inspect`` scans
...
def _populate_cached_file_ids(image_refs):
    ...
    for (repository, image_name, tag), ref in unique_keys.items():
        ...
        if results:
            ...
            props_digest = props.get("image_digest", "")

            stored_digest = props_digest
