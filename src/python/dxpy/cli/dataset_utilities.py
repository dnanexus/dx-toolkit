...
def extract_dataset(args):
    ...
    if args.fields is not None:
        ...
        if args.sql:
            resource_val = (
                resp["url"] + "/viz-query/3.0/" + resp["dataset"] + "/raw-query"
            )
            try:
...
if __name__ == "__main__":
    record = dxpy.DXRecord(dxid="record-GJVgv60J8vzFQzq7Fvk44y4q")
    record.describe(default_fields=True, fields={"properties", "details"})
