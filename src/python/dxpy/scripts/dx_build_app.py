...
def _get_timestamp_version_suffix():
    return "+build." + datetime.today().strftime('%Y%m%d.%H%M')

def _get_version_suffix(src_dir):
    ...
    try:
        ...
        pass
    return _get_timestamp_version_suffix()
        ...
        if do_version_autonumbering:
            ...
            if app_describe is not None:
                if app_describe.has_key('published') or not do_try_update:
                    ...
                    build_options['version_override'] = original_version + _get_version_suffix(src_dir)
    ...
    try:
        ...
        if mode == "app":
            ...
            if not version_override and do_version_autonumbering:
                try_versions.append(version + _get_version_suffix(src_dir))
