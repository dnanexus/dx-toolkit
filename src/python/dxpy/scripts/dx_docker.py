        ...
        def wrapper(*args, **kwargs):
            ...
            for i in range(tries):
                try:
                    ...
                    sys.stderr.write(dxpy.utils.printing.YELLOW("Error. Retrying in {} s.\n".format(mdelay)))
                    time.sleep(mdelay)
                    mdelay *= backoff
                    if i == tries - 1:
...
def pull(image, quiet=None):
    ...
    get_aci(image, "docker://"+image, quiet=quiet)
...
def run(image, command=[], quiet=None, rm=True, env=[], rootfs=None, workdir=None, volumes=[], entrypoint=None):
    ...
    if entrypoint:
        ...
        entry_point = json.loads(annotations['entrypoint'])
    ...
    if 'app' in imagemeta and 'workingDirectory' in imagemeta['app']:
       wkdir = imagemeta['app']['workingDirectory']
    if workdir:
       wkdir = workdir

    default_homedir_env = "HOME=/root"
    environment = ["env", "-i", default_homedir_env]
    if 'app' in imagemeta and 'environment' in imagemeta['app']:
        environment.extend(env + [var['name'] +"="+var['value']  for var in imagemeta['app']['environment']])
    ...
    proot_cmd =  ["proot", "-R", "{tmpdir}/rootfs".format(tmpdir=tmpdir)] + volume + ["-w", wkdir] + environment + ["/bin/sh", "-c"] + [" ".join(("'"+x+"'" for x in container_cmd))]
    shell(proot_cmd)
    if not rootfs:
        shutil.rmtree(tmpdir)
...
def add_to_applet(applet, image, alternative_export=False):
    ...
    tmpdir = tempfile.mkdtemp()
    local_image_to_aci(image, tmpdir, os.path.abspath(applet)+"/resources", alternative_export)
    shutil.rmtree(tmpdir)
...
def create_asset(image, output_path=None, alternative_export=False):
    ...
    local_image_to_aci(image, tmpdir, tmpdir+"/resources", alternative_export)

    imagename = image.replace("/", "%").replace(":","#")
    # Create the asset configuration
    dxasset = { 'name': imagename }
    dxasset['title'] = "DNAnexus asset for Docker image "+image
    ...
    if output_path:
        if ":" in output_path:
            project_id, folder_path = output_path.split(":")
        else:
    ...
    escaped_image_name = image.replace("/","\\/").replace(":", "\\:")
    ...
    sys.stderr.write("    {"+dxpy.utils.printing.BLUE("""
        ...
        "name": \"{}\",
        "version": "0.0.1"
    """).format(project_id, folder_path, image)+"}\n")
