...
def ssh(args, ssh_config_verified=False):
    ...
    for i in range(90):
        ...
        host_key = job_desc.get("sshHostKey") or job_desc["properties"].get(
            "ssh_host_rsa_key"
        )
    ...
    with open(known_hosts_file, "a") as fh:
        fh.write(
            "{job_id}.dnanex.us {key}\n".format(
                job_id=args.job_id, key=host_key.rstrip()
            )
        )
    ...
    ssh_args = [
        ...
        "-o",
        "HostKeyAlias={}.dnanex.us".format(args.job_id),
        "-o",
...
parser_describe.add_argument(
    "--verbose", help="Include all possible metadata", action="store_true"
)
parser_describe.add_argument(
...
parser_run.add_argument(
    ...
    "--rank",
    type=int,
    help="Set the rank of the root execution, integer between -1024 and 1023. Requires executionRankEnabled license feature for the billTo. Default is 0.",
    default=None,
)
...
parser_extract_dataset = subparsers.add_parser(
    "extract_dataset",
    help="Retrieves the data or generates SQL to retrieve the data from a dataset or cohort for a set of entity.fields. Additionally, the dataset's dictionary can be extracted independently or in conjunction with data.",
