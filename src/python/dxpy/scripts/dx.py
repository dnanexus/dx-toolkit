...
from ..cli.dataset_utilities import (
    extract_dataset,
    extract_assay_germline,
    extract_assay_somatic,
)
...
from ..cli.parsers import (
    ...
    all_arg,
    json_arg,
    parser_dataobject_args,
    parser_single_dataobject_output_args,
    process_properties_args,
    find_by_properties_and_tags_args,
    process_find_by_property_args,
    process_dataobject_args,
    process_single_dataobject_output_args,
    find_executions_args,
    ...
    process_instance_count_arg,
    get_update_project_args,
    property_args,
    tag_args,
    contains_phi,
    process_phi_param,
    process_external_upload_restricted_param,
)
...
if "_ARGCOMPLETE" not in os.environ:
    try:
        ...
        if os.name != "nt":
            print(
                "Warning: readline module is not available, tab completion disabled",
                file=sys.stderr,
            )

state = {"interactive": False, "colors": "auto", "delimiter": None, "currentproj": None}
parser_map = {}
parser_categories_sorted = [
    "all",
    "session",
    "fs",
    "data",
...
parser_categories = {
    "all": {"desc": "\t\tAll commands", "cmds": []},
    "session": {"desc": "\tManage your login session", "cmds": []},
    "fs": {"desc": "\t\tNavigate and organize your projects and files", "cmds": []},
    "data": {"desc": "\t\tView, download, and upload data", "cmds": []},
    "metadata": {
        "desc": "\tView and modify metadata for projects, data, and executions",
        "cmds": [],
    },
    "workflow": {"desc": "\tView and modify workflows", "cmds": []},
    "exec": {"desc": "\t\tManage and run apps, applets, and workflows", "cmds": []},
    "org": {"desc": "\t\tAdminister and operate on orgs", "cmds": []},
    "other": {"desc": "\t\tMiscellaneous advanced utilities", "cmds": []},
}
...
class DXCLICompleter:
    ...
    def __init__(self):
        self.commands = [
            subcmd + " "
            for subcmd in list(subparsers.choices.keys())
            if subcmd not in self.silent_commands
        ]
        self.matches = []
        self.text = None
    ...
    def get_subcommand_matches(self, command, prefix):
        if command in self.subcommands:
            self.matches = [
                command + " " + sub
                for sub in self.subcommands[command]
    ...
    def get_matches(self, text, want_prefix=False):
        ...
        if num_words == 0:
            ...
            if words[0] == "run":
                path_matches = path_completer(
                    words[-1], classes=["applet", "workflow"], visibility="visible"
                )
            elif words[0] in ["cd", "rmdir", "mkdir", "tree"]:
                path_matches = path_completer(words[-1], expected="folder")
            elif words[0] in ["head", "cat", "download"]:
                path_matches = path_completer(words[-1], classes=["file"])
                ...
                path_matches = path_completer(
                    words[-1], expected="project", include_current_proj=True
                )
            else:
                path_matches = []
            ...
            if words[0] in ["run", "install", "uninstall"]:
                try:
                    initial_results = list(
                        dxpy.find_apps(
                            describe={"fields": {"name": True, "installed": True}}
                        )
                    )
                    if words[0] in ["run", "uninstall"]:
                        filtered_results = [
                            ...
                            if result["describe"]["installed"]
                        ]
                    else:
                        filtered_results = [
                            result
                            for result in initial_results
                            if not result["describe"]["installed"]
                        ]
                    app_names = [
                        result["describe"]["name"] for result in filtered_results
                    ]
                    app_matches = [
                        name for name in app_names if name.startswith(words[-1])
                    ]
                    if want_prefix:
                        self.matches += [
                            text[: space_pos + 1] + match for match in app_matches
                        ]
                    else:
...
def login(args):
    ...
    if args.token is None:
        if (args.host is None) != (args.port is None):
            err_exit(
                "Error: Only one of --host and --port were provided; provide either both or neither of the values",
                2,
            )
        ...
        def get_token(**data):
            return dxpy.DXHTTPRequest(
                authserver + "/system/newAuthToken",
                data,
                prepend_srv=False,
                auth=None,
                always_retry=True,
            ...
            if reuse:
                ...
                while not username:
                    if "DX_USERNAME" in os.environ:
                        username = (
                            input("Username [" + os.environ["DX_USERNAME"] + "]: ")
                            or os.environ["DX_USERNAME"]
                        )
                    else:
        ...
        while attempt <= 3:
            try:
                ...
                token_res = get_token(
                    expires=normalize_time_input(
                        args.timeout, future=True, default_unit="s"
                    ),
                    **credentials
                )
                break
                ...
                if e.name == "OTPRequiredError":
                    ...
                    if attempt < 3:
                        ...
                        err_exit(
                            "Incorrect username and/or password", arg_parser=parser
                        )
                else:
        ...
        sec_context = json.dumps(
            {
                "auth_token": token_res["access_token"],
                "auth_token_type": token_res["token_type"],
            }
        )

        if using_default:
            set_api(
                dxpy.DEFAULT_APISERVER_PROTOCOL,
                dxpy.DEFAULT_APISERVER_HOST,
                dxpy.DEFAULT_APISERVER_PORT,
                args.save,
            )
    else:
    ...
    if args.token is not None:
        ...
        try:
            ...
            print(
                "Could not obtain username from auth server. Consider setting both --host and --port.",
                file=sys.stderr,
            )
        ...
        try:
            ...
            if greeting.get("messages"):
                ...
                for message in greeting["messages"]:
                    ...
                    print(
                        BOLD("Subject: ")
                        + fill(message["title"], subsequent_indent=" " * 9)
                    )
    ...
    if args.save and not args.token:
        ...
        tip = (
            "Use "
            + BOLD("dx login --timeout")
            + " to control the expiration date, or "
            + BOLD("dx logout")
            + " to end this session."
        )
        timeout = datetime.timedelta(
            seconds=normalize_time_input(args.timeout, default_unit="s") // 1000
        )
        print(
            fill(
                msg.format(
                    conf_dir=dxpy.config.get_user_conf_dir(), timeout=timeout, tip=tip
                )
            )
        )
    ...
    if dxpy.AUTH_HELPER is not None:
        ...
        try:
            ...
            response = dxpy.DXHTTPRequest(
                authserver + "/system/destroyAuthToken",
...
def set_api(protocol, host, port, write):
    dxpy.config.update(
        DX_APISERVER_PROTOCOL=protocol, DX_APISERVER_HOST=host, DX_APISERVER_PORT=port
    )
    if write:
        dxpy.config.save()
...
def pick_and_set_project(args):
    try:
        result_generator = dxpy.find_projects(
            describe=True,
            name=args.name,
            name_mode="glob",
    ...
    while True:
        ...
        if not any_results:
            parser.exit(
                ...
                "\n"
                + fill(
                    "No projects to choose from.  You can create one with the command "
                    + BOLD("dx new project")
                    + ".  To pick from projects for which you only have "
                    + " VIEW permissions, use "
                    + BOLD("dx select --level VIEW")
                    + " or "
        ...
        if choice == "m":
            ...
            print(
                ...
                + BOLD(results[choice]["describe"]["name"])
            )
            set_project(
                project_ids[choice],
                not state["interactive"] or args.save,
                name=results[choice]["describe"]["name"],
            )
...
def setenv(args):
    ...
    if args.current:
        ...
        try:
            api_protocol = prompt_for_env_var(
                'API server protocol (choose "http" or "https")',
...
def env(args):
    if args.bash:
        if dxpy.AUTH_HELPER is not None:
            print(
                "export DX_SECURITY_CONTEXT='"
                + json.dumps(dxpy.AUTH_HELPER.security_context)
                + "'"
            )
        ...
        if dxpy.AUTH_HELPER is not None:
            print(
                "Auth token used\t\t"
                + dxpy.AUTH_HELPER.security_context.get("auth_token", "none")
            )
        ...
        if "DX_PROJECT_CONTEXT_NAME" in os.environ:
            print(
                'Current workspace name\t"{n}"'.format(
                    n=dxpy.config.get("DX_PROJECT_CONTEXT_NAME")
                )
            )
...
def api(args):
    ...
    resp = None
    try:
        resp = dxpy.DXHTTPRequest("/" + args.resource + "/" + args.method, json_input)
    except:
        err_exit()
    try:
...
def invite(args):
    ...
    if ":" not in args.project:
        args.project += ":"
    project, _none, _none = try_call(resolve_existing_path, args.project, "project")
...
def select(args):
    if args.project is not None:
        ...
        print(
            "Selected project",
            split_unescaped(":", args.project)[0].replace("\\:", ":"),
        )
    else:
        pick_and_set_project(args)
...
def cd(args):
    ...
    try:
        ...
        err_exit(
            fill(
                folderpath
                + ": No such file or directory found in project "
                + dxpy.WORKSPACE_ID
            ),
            3,
        )
...
def ls(args):
    project, folderpath, entity_results = try_call(
        resolve_existing_path,  # TODO: this needs to honor "ls -a" (all) (args.obj/args.folders/args.full)
        args.path,
    ...
    if project is None:
        err_exit(
            "Current project must be set or specified before any data can be listed", 3
        )
    ...
    if entity_results is None:
            ...
            if not args.folders:
                ...
                if not args.brief and not args.verbose:
                    name_counts = collections.Counter(
                        obj["describe"]["name"] for obj in resp["objects"]
                    )
                for obj in resp["objects"]:
                    if args.brief:
                        ...
                        print_ls_desc(
                            obj["describe"],
                            print_id=True
                            if name_counts[obj["describe"]["name"]] > 1
                            else False,
                        )
        except:
            err_exit()
        ...
        name_counts = collections.Counter(
            obj["describe"]["name"] for obj in entity_results
        )
            ...
            if result["describe"]["project"] == project:
                if args.brief:
                    ...
                    print_ls_desc(
                        result["describe"],
                        print_id=True
                        if name_counts[result["describe"]["name"]] > 1
                        else False,
                    )
    ...
    for path in args.paths:
        ...
        try:
            dxpy.api.project_new_folder(
                project, {"folder": folderpath, "parents": args.parents}
            )
...
def rmdir(args):
    ...
    for path in args.paths:
        ...
        try:
            completed = False
            while not completed:
                resp = dxpy.api.project_remove_folder(
                    project, {"folder": folderpath, "partial": True}
                )
...
def rm(args):
    ...
    if args.recursive and not args.force:
        for path in args.paths:
            try:
                with nostderr():
                    project, folderpath, entity_results = resolve_existing_path(
                        path, allow_mult=True, all_mult=args.all
                    )
                if folderpath == "/" and entity_results is None:
                    ...
                    print(
                        "*     {}: Recursive deletion will remove all files in project!     *".format(
                            RED("RED ALERT")
                        )
                    )
                    print(
                    ...
                    print(
                        "*   Please issue 'dx rm -r --force' if you are sure you want to do this.  *"
                    )
                    print(
                        "==========================================================================="
                    )
            ...
            if folderpath is not None:
                if not args.recursive:
                    print(
                        fill(
                            'Did not find "'
                            + path
                            + '" as a data object; if it is a folder, cannot remove it without setting the "-r" flag'
                        )
                    )
                    had_error = True
                    continue
                ...
                print(
                    fill(
                        "Path "
                        + path
                        + ' resolved to a project; cannot remove a project using "rm"'
                    )
                )
                had_error = True
                continue
    ...
    for project in projects:
        ...
        try:
            ...
            print(
                ...
                + json.dumps(projects[project]["objects"])
                + " from "
                + project
            )
            print("  " + str(details))
    ...
    for project in args.projects:
        ...
        if len(substrings) > 1 or (len(substrings) == 1 and project[0] == ":"):
            print(
                fill(
                    'Unable to remove "'
                    + project
                    + '": a nonempty string was found to the right of an unescaped colon'
                )
            )
            had_error = True
            continue
        if len(substrings) == 0:
            if project[0] == ":":
                print(
                    fill(
                        'Unable to remove ":": to remove the current project, use its name or ID'
                    )
                )
                had_error = True
                continue
        ...
        if proj_id is None:
            print(
                fill(
                    'Unable to remove "'
                    + project
                    + '": could not resolve to a project ID'
                )
            )
            had_error = True
            continue
        try:
            ...
            if args.confirm:
                ...
                if len(value) == 0 or value.lower()[0] != "y":
                    had_error = True
                    print(
                        fill('Aborting deletion of project "' + proj_desc["name"] + '"')
                    )
                    continue
            try:
                ...
                if apierror.name == "InvalidState":
                    value = input(
                        fill("WARNING: there are still unfinished jobs in the project.")
...
def mv(args):
    dest_proj, dest_path, _none = try_call(
        resolve_path, args.destination, expected="folder"
    )
    try:
        ...
        src_proj, src_path, src_results = try_call(
            resolve_existing_path, args.sources[0], allow_mult=True, all_mult=args.all
        )

        if src_proj != dest_proj:
            err_exit(
                fill(
                    'Error: Using "mv" for moving something from one project to another is unsupported.'
                ),
                3,
            )
        ...
        if src_results is None:
            if src_path == "/":
                err_exit(
                    fill(
                        'Cannot rename root folder; to rename the project, please use the "dx rename" subcommand.'
                    ),
                    3,
                )
            try:
                dxpy.api.project_rename_folder(
                    src_proj, {"folder": src_path, "newpath": dest_path}
                )
                return
            except:
                err_exit()
            ...
            try:
                ...
                for result in src_results:
                    dxpy.DXHTTPRequest(
                        "/" + result["id"] + "/rename",
                        {"project": src_proj, "name": dest_name},
                    )
                return
            except:
                err_exit()
    ...
    for source in args.sources:
        src_proj, src_folderpath, src_results = try_call(
            resolve_existing_path, source, allow_mult=True, all_mult=args.all
        )
        if src_proj != dest_proj:
            err_exit(
                fill(
                    'Using "mv" for moving something from one project to another is unsupported.  Please use "cp" and "rm" instead.'
                ),
                3,
            )
    ...
    try:
        dxpy.api.project_move(
            src_proj,
            {"objects": src_objects, "folders": src_folders, "destination": dest_path},
        )
    except:
        err_exit()
...
def tree(args):
    project, folderpath, _none = try_call(
        resolve_existing_path, args.path, expected="folder"
    )

    if project is None:
        err_exit(
            fill(
                "Current project must be set or specified before any data can be listed"
            ),
            3,
        )
    dxproj = dxpy.get_handler(project)
    ...
    try:
        folders = [
            folder
            for folder in dxproj.describe(input_params={"folders": True})["folders"]
...
def describe(args):
    ...
    def find_global_executable(json_output, args):
        ...
        if not found_match:
            for result in dxpy.find_global_workflows(name=args.path, describe=True):
                result["describe"] = dxpy.append_underlying_workflow_describe(
                    result["describe"]
                )
                append_to_output_json_and_print(result)
                found_match = True
    ...
    try:
        ...
        if args.name and (args.verbose or args.details or args.json):
            raise DXCLIError(
                "Cannot request --name in addition to one of --verbose, --details, or --json"
            )
        ...
        # execution
        try:
            project, _folderpath, entity_results = resolve_existing_path(
                args.path, expected="entity", ask_to_resolve=False, describe=json_input
            )
        except ResolutionError as details:
        ...
        if not is_hashid(args.path) and ":" not in args.path:
            ...
            if args.path.startswith("app-") or args.path.startswith("globalworkflow-"):
                found = describe_global_executable(
                    json_output, args, args.path.partition("-")[0]
                )
            else:
            ...
            if args.path.startswith("user-"):
                # User
                try:
                    desc = dxpy.api.user_describe(
                        args.path, {"appsInstalled": True, "subscriptions": True}
                    )
                    found_match = True
                    if args.json:
                        json_output.append(desc)
        ...
        if args.json:
            if args.multi:
                ...
                raise DXCLIError(
                    ...
                    + args.path
                    + "; to get all of them in JSON format, also provide the --multi flag."
                )
...
def _validate_new_user_input(args):
    ...
    if args.org is None and len(DXNewUserOrgArgsAction.user_specified_opts) > 0:
        raise DXCLIError(
            "Cannot specify {opts} without specifying --org".format(
...
def _get_user_new_args(args):
    ...
    """
    user_new_args = {"username": args.username, "email": args.email}
...
def new_user(args):
    ...
    dxpy.DXHTTPRequest(
        dxpy.get_auth_server_name() + "/user/new",
...
def new_project(args):
    if args.name == None:
        if INTERACTIVE_CLI:
            ...
            err_exit(
                parser_new_project.format_help()
    ...
    try:
        ...
        if args.select or (
            INTERACTIVE_CLI
            and prompt_for_yn("Switch to new project now?", default=False)
        ):
...
def new_record(args):
    ...
    try:
        ...
        if args.brief:
            ...
            print_desc(
                dxrecord.describe(incl_properties=True, incl_details=True), args.verbose
            )
    except:
        err_exit()
...
def set_visibility(args):
    had_error = False
    # Attempt to resolve name
    _project, _folderpath, entity_results = try_call(
        resolve_existing_path,
        args.path,
        expected="entity",
        allow_mult=True,
...
def get_details(args):
    # Attempt to resolve name
    _project, _folderpath, entity_result = try_call(
        resolve_existing_path, args.path, expected="entity"
    )
    ...
    try:
        print(
            json.dumps(
                dxpy.DXHTTPRequest("/" + entity_result["id"] + "/getDetails", {}),
                indent=4,
            )
        )
    except:
...
def set_details(args):
    ...
    if entity_results is None:
        err_exit(
            exception=ResolutionError(
                'Could not resolve "' + args.path + '" to a name or ID'
            ),
    ...
    if args.details is not None and args.details_file is not None:
        err_exit(
            exception=DXParserError(
                "Cannot provide both -f/--details-file and details"
            ),
        ...
        try:
            ...
            err_exit(
                "Error: Details could not be parsed as JSON",
        ...
        with (
            sys.stdin if args.details_file == "-" else open(args.details_file, "r")
        ) as fd:
            data = fd.read()
            try:
    ...
    # Throw error if missing arguments.
        ...
        err_exit(
            exception=DXParserError("Must set one of -f/--details-file or details"),
            expected_exceptions=(DXParserError,),
        )
...
def add_types(args):
    ...
    for result in entity_results:
        try:
            dxpy.DXHTTPRequest("/" + result["id"] + "/addTypes", {"types": args.types})
        except (dxpy.DXAPIError,) + network_exceptions as details:
            print(format_exception(details), file=sys.stderr)
            had_error = True
    if had_error:
...
def remove_types(args):
    ...
    for result in entity_results:
        try:
            dxpy.DXHTTPRequest(
                "/" + result["id"] + "/removeTypes", {"types": args.types}
            )
        except (dxpy.DXAPIError,) + network_exceptions as details:
            print(format_exception(details), file=sys.stderr)
            had_error = True
...
def add_tags(args):
    had_error = False
    # Attempt to resolve name
    project, _folderpath, entity_results = try_call(
        resolve_to_objects_or_project, args.path, args.all
    )
    ...
    if entity_results is not None:
        for result in entity_results:
            try:
                dxpy.DXHTTPRequest(
                    "/" + result["id"] + "/addTags",
                    {"project": project, "tags": args.tags},
                )
            except (dxpy.DXAPIError,) + network_exceptions as details:
                print(format_exception(details), file=sys.stderr)
                had_error = True
        ...
        try:
            dxpy.DXHTTPRequest("/" + project + "/addTags", {"tags": args.tags})
        except:
            err_exit()
...
def remove_tags(args):
    ...
    if entity_results is not None:
        ...
        try:
            dxpy.DXHTTPRequest("/" + project + "/removeTags", {"tags": args.tags})
        except:
            err_exit()
...
def set_properties(args):
    ...
    if entity_results is not None:
        for result in entity_results:
            try:
                dxpy.DXHTTPRequest(
                    "/" + result["id"] + "/setProperties",
                    {"project": project, "properties": args.properties},
                )
            except (dxpy.DXAPIError,) + network_exceptions as details:
                print(format_exception(details), file=sys.stderr)
                had_error = True
...
def make_download_url(args):
    project, _folderpath, entity_result = try_call(
        resolve_existing_path, args.path, expected="entity"
    )
    if entity_result is None:
    ...
    try:
        ...
        project = (
            dxfile.project
            if re.match(r"^project-[a-zA-Z0-9]{24}$", dxfile.project)
            else dxpy.DXFile.NO_PROJECT_HINT
        )
        url, _headers = dxfile.get_download_url(
            preauthenticated=True,
            duration=normalize_timedelta(args.duration) // 1000
            if args.duration
            else 24 * 3600,
...
def get_output_path(obj_name, obj_class, args):
    ...
    if args.output == "-":
        err_exit(
            "Error: {} ".format(obj_class)
            + "objects cannot be dumped to stdout, please specify a directory",
            3,
        )
...
def get_applet(project, entity_result, args):
    ...
    output_path = get_output_path(obj_name, entity_result["describe"]["class"], args)
    ...
    describe_output = dx_obj.describe(incl_properties=True, incl_details=True)
    dump_executable(
        dx_obj,
        output_path,
...
def get_workflow(entity_result, args):
    ...
    dump_executable(
        dx_obj, output_path, omit_resources=True, describe_output=describe_output
    )
...
def get_database(entity_result, args):
    ...
    output_path = get_output_path(obj_id, entity_result["describe"]["class"], args)
    ...
    if args.filename is None or args.filename == "/" or args.filename == "":
        if not args.allow_all_files:
            err_exit(
                "Error: downloading all files from a database not allowed unless --allow-all-files argument is specified.",
                3,
            )
...
def get(args):
    ...
    if (
        not is_hashid(args.path)
        and ":" not in args.path
        and args.path.startswith("app-")
    ):
        desc = dxpy.api.app_describe(args.path)
        ...
        not is_hashid(args.path)
        and ":" not in args.path
        and args.path.startswith("globalworkflow-")
    ):
        desc = dxpy.api.global_workflow_describe(args.path)
        entity_result = {"id": desc["id"], "describe": desc}
    else:
    ...
    if entity_result_class == "file":
        download_one_file(
            project, entity_result["describe"], entity_result["describe"]["name"], args
        )
        ...
        err_exit(
            ...
            + " but an object of class file, record, applet, app, or workflow was expected",
            3,
        )
...
def cat(args):
    for path in args.path:
        ...
        if (
            path_has_explicit_proj
            and project is not None
            and not object_exists_in_project(entity_result["describe"]["id"], project)
        ):
        ...
        try:
            ...
            while True:
                ...
                chunk = dxfile.read(
                    1024 * 1024, project=project or dxpy.DXFile.NO_PROJECT_HINT
                )
                if len(chunk) == 0:
                    break
                if mode == "rb":
...
def head(args):
    ...
    if not entity_result["describe"]["class"] in ["file"]:
        err_exit(
            "Error: The given object is of class "
            + entity_result["describe"]["class"]
            + " but an object of class file was expected",
            3,
        )
...
def upload(args, **kwargs):
    if args.output is not None and args.path is not None:
        raise DXParserError(
            "Error: Cannot provide both the -o/--output and --path/--destination arguments"
        )
...
def upload_one(args):
    ...
    if os.path.isdir(args.filename):
        if not args.recursive:
            err_exit(
                "Error: {f} is a directory but the -r/--recursive option was not given".format(
                    f=args.filename
                ),
                3,
            )
        ...
        if norm_path in upload_seen_paths:
            print(
                "Skipping {f}: directory loop".format(f=args.filename), file=sys.stderr
            )
            return
        else:
        ...
        if len(dir_listing) == 0:  # Create empty folder
            dxpy.api.project_new_folder(
                project,
                {
                    "folder": os.path.join(folder, os.path.basename(args.filename)),
                    "parents": True,
                },
            )
        else:
            for f in dir_listing:
                ...
                sub_args.path = "{p}:{f}/{sf}/".format(
                    p=project, f=folder, sf=os.path.basename(args.filename)
                )
        ...
        try:
            dxfile = dxpy.upload_local_file(
                filename=(None if args.filename == "-" else args.filename),
...
def find_executions(args):
    ...
    if args.id is not None:
        ...
        if args.project is not None:
            ...
            project, _none, _none = try_call(
                resolve_existing_path, args.project, "project"
            )
        if (
            args.user is not None
            and args.user != "self"
            and not args.user.startswith("user-")
        ):
    ...
    def build_tree(
        root, executions_by_parent, execution_descriptions, is_cached_result=False
    ):
        tree, root_string = {}, ""
        if args.json:
            ...
            root_string = get_find_executions_string(
                execution_descriptions[root],
        ...
        for child_execution in executions_by_parent.get(root, {}):
            child_is_cached_result = is_cached_result or (
                execution_descriptions[child_execution].get("outputReusedFrom")
                is not None
            )
            subtree, _subtree_root = build_tree(
                child_execution,
    ...
    def process_tree(result, executions_by_parent, execution_descriptions):
        ...
        tree, root = build_tree(
            result["id"], executions_by_parent, execution_descriptions, is_cached_result
        )
        if tree:
    ...
    try:
        ...
        if args.trees:
            ...
            def process_execution_result(execution_result):
                ...
                parent = execution_desc.get(parent_field) or execution_desc.get(
                    "parentAnalysis"
                )
                ...
                if execution_desc["class"] == "analysis":
                    for stage_desc in execution_desc["stages"]:
                        if (
                            "parentAnalysis" in stage_desc["execution"]
                            and stage_desc["execution"]["parentAnalysis"]
                            != execution_result["id"]
                            and (
                                args.classname != "analysis"
                                or stage_desc["execution"]["class"] == "analysis"
                            )
                        ):
                            # this is a cached stage (with a different parent)
                            executions_by_parent[execution_result["id"]].append(
                                stage_desc["execution"]["id"]
                            )
                            if stage_desc["execution"]["id"] not in descriptions:
                                descriptions[
                                    stage_desc["execution"]["id"]
                                ] = stage_desc["execution"]
            ...
            if len(list(roots.keys())) > 0:
                ...
                sorted_roots = sorted(
                    roots, key=lambda root: -descriptions[roots[root]]["created"]
                )

                for root in sorted_roots:
                    process_tree(
                        descriptions[roots[root]], executions_by_parent, descriptions
                    )
        if args.json:
            print(json.dumps(json_output, indent=4))
        ...
        if more_results and get_delimiter() is None and not (args.brief or args.json):
            print(
                fill(
                    "* More results not shown; use -n to increase number of results or --created-before to show older results",
                    subsequent_indent="  ",
                )
            )
    except:
...
def find_data(args):
    ...
    if args.folder is None and args.path is not None:
        ...
        err_exit(
            exception=DXParserError("Cannot supply both --folder and --path."),
            expected_exceptions=(DXParserError,),
        )
    ...
    if args.all_projects:
        ...
        if args.folder is not None and get_last_pos_of_char(":", args.folder) != -1:
            err_exit(
                exception=DXParserError(
                    "Cannot supply both --project and --path PROJECTID:FOLDERPATH."
                ),
                expected_exceptions=(DXParserError,),
            )

        args.project, _none, _none = try_call(
            resolve_existing_path, args.project, "project"
        )

    if args.folder is not None and not args.folder.startswith("/"):
        args.project, args.folder, _none = try_call(
            resolve_path, args.folder, expected="folder"
        )
            ...
            for result in results:
                if args.verbose:
                    ...
                    print_ls_l_desc(
                        result["describe"],
                        include_folder=True,
                        include_project=args.all_projects,
                    )
    except:
        err_exit()
...
def find_apps_result(args):
    ...
    if args.installed:
        maybe_filtered_by_install = (
            result for result in raw_results if result["describe"]["installed"]
        )
    else:
    ...
    if args.brief:
        ...
        results = sorted(
            maybe_filtered_by_install, key=lambda result: result["describe"]["name"]
        )
    return results


def find_global_workflows_result(args):
    raw_results = dxpy.find_global_workflows(
        name=args.name,
        name_mode="glob",
        category=args.category,
        all_versions=args.all,
...
def print_find_results(results, args):
    def maybe_x(result):
        return (
            DNANEXUS_X()
            if result["describe"]["billTo"] in ["org-dnanexus", "org-dnanexus_apps"]
            else " "
        )
    ...
    if args.brief:
        ...
        for result in results:
            print(
                maybe_x(result)
                + DELIMITER(" ")
                + result["describe"].get("title", result["describe"]["name"])
                + DELIMITER(" (")
                + result["describe"]["name"]
                + DELIMITER("), v")
                + result["describe"]["version"]
            )
    else:
        for result in results:
            print(
                ...
                + (
                    "published"
                    if result["describe"].get("published", 0) > 0
                    else "unpublished"
                )
                + DELIMITER(")")
            )
...
def update_project(args):
    ...
    project, _none, _none = try_call(resolve_existing_path, args.project_id, "project")
    try:
...
def close(args):
    ...
    for path in args.path:
        ...
        try:
            project, _folderpath, entity_results = resolve_existing_path(
                path, expected="entity", allow_mult=True, all_mult=args.all
            )
        except:
            project, entity_results = None, None
...
def wait(args):
    ...
    for path in args.path:
        if is_job_id(path) or is_analysis_id(path):
            ...
            # Attempt to resolve name
            try:
                project, _folderpath, entity_result = resolve_existing_path(
            ...
            if entity_result is None:
                ...
                handler = dxpy.get_handler(
                    entity_result["id"], project=entity_result["describe"]["project"]
                )
...
def build(args):
    ...
    def get_source_exec_desc(source_exec_path):
        ...
        :rtype: dict
        """
        exec_describe_fields = {
            "fields": {"properties": True, "details": True},
            "defaultFields": True,
        }
        _, _, exec_result = try_call(
            resolve_existing_path,
            source_exec_path,
            expected="entity",
            ask_to_resolve=False,
        ...
        if exec_result is None:
            err_exit(
                "Could not resolve {} to an existing applet or workflow.".format(
                    source_exec_path
                ),
                3,
            )
            ...
            err_exit(
                "More than one match found for {}. Please use an applet/workflow ID instead.".format(
                    source_exec_path
                ),
                3,
            )
        else:
            if exec_result[0]["id"].startswith("applet") or exec_result[0][
                "id"
            ].startswith("workflow"):
                return exec_result[0]["describe"]
            else:
                err_exit(
                    "Could not resolve {} to a valid applet/workflow ID".format(
                        source_exec_path
                    ),
                    3,
                )
    ...
    def get_validated_source_dir(args):
        if args._from is not None:
            if args.src_dir is not None:
                build_parser.error(
                    "Source directory and --from cannot be specified together"
                )
            return None
    ...
    def handle_arg_conflicts(args):
        ...
        if args.mode == "globalworkflow" and args.destination != ".":
            build_parser.error(
                "--destination cannot be used when creating a global workflow (only a workflow)"
            )

        if args.mode == "applet" and args.region:
            build_parser.error(
                "--region cannot be used when creating an applet (only an app)"
            )
        ...
        if args.run is not None and args.dry_run:
            build_parser.error(
                "Options --dry-run and --run cannot be specified together"
            )

        if args.run and args.remote and args.mode == "app":
            build_parser.error(
                "Options --remote, --app, and --run cannot all be specified together. Try removing --run and then separately invoking dx run."
            )
        ...
        if args._from is not None and args.ensure_upload:
            build_parser.error(
                "Options --from and --ensure-upload cannot be specified together"
            )

        if args._from is not None and args.force_symlinks:
            build_parser.error(
                "Options --from and --force-symlinks cannot be specified together"
            )

        if args._from is not None and args.remote:
            build_parser.error(
                "Options --from and --remote cannot be specified together"
            )

        if args._from is not None and not args.parallel_build:
            build_parser.error(
                "Options --from and --no-parallel-build cannot be specified together"
            )

        if args._from is not None and (
            args.mode != "app" and args.mode != "globalworkflow"
        ):
            build_parser.error(
                "--from can only be used to build an app from an applet or a global workflow from a project-based workflow"
            )

        if args._from is not None and not args.version_override:
            build_parser.error(
                "--version must be specified when using the --from option"
            )

        if (
            args.mode == "app"
            and args._from is not None
            and not args._from["id"].startswith("applet")
        ):
            build_parser.error(
                "app can only be built from an applet (--from should be set to an applet ID)"
            )

        if (
            args.mode == "globalworkflow"
            and args._from is not None
            and not args._from["id"].startswith("workflow")
        ):
            build_parser.error(
                "globalworkflow can only be built from an workflow (--from should be set to a workflow ID)"
            )

        if args._from and args.dry_run:
            build_parser.error(
                "Options --dry-run and --from cannot be specified together"
            )
        ...
        if args.repository and not args.nextflow:
            build_parser.error(
                "Repository argument is available only when building a Nextflow pipeline. Did you mean 'dx build --nextflow'?"
            )

        if args.repository and args.remote:
            build_parser.error(
                "Nextflow pipeline built from a remote Git repository is always built using the Nextflow Pipeline Importer app. This is not compatible with --remote."
            )

        if args.git_credentials and not args.repository:
            build_parser.error(
                "Git credentials can be supplied only when building Nextflow pipeline from a Git repository."
            )

        if args.nextflow and args.mode == "app":
            build_parser.error(
                "Building Nextflow apps is not supported. Build applet instead."
            )
        ...
        if args.mode == "workflow":
            ...
            used_unsupported_options = {
                k: v for k, v in list(unsupported_options.items()) if v
            }
            if used_unsupported_options:
                build_parser.error(
                    "Options {} are not supported with workflows".format(
                        ", ".join(used_unsupported_options)
                    )
                )
...
def add_users(args):
    ...
    try:
        if desc["class"] == "app":
            dxpy.api.app_add_authorized_users(
                desc["id"], input_params={"authorizedUsers": args.users}
            )
        else:
            dxpy.api.global_workflow_add_authorized_users(
                desc["id"], input_params={"authorizedUsers": args.users}
            )
    except:
        err_exit()
...
def remove_users(args):
    ...
    try:
        if desc["class"] == "app":
            ...
            dxpy.api.global_workflow_remove_authorized_users(
                desc["id"], input_params={"authorizedUsers": args.users}
            )
    except:
        err_exit()
    ...
    try:
        if desc["class"] == "app":
            dxpy.api.app_add_developers(
                desc["id"], input_params={"developers": args.developers}
            )
        else:
            dxpy.api.global_workflow_add_developers(
                desc["id"], input_params={"developers": args.developers}
            )
    except:
        err_exit()
...
def list_developers(args):
    ...
    try:
        if desc["class"] == "app":
            ...
            developers = dxpy.api.global_workflow_list_developers(desc["id"])[
                "developers"
            ]
...
def render_timestamp(epochSeconds):
    ...
    return datetime.datetime.fromtimestamp(epochSeconds // 1000).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
...
def list_database_files(args):
    try:
        ...
        if is_hashid(args.database):
            ...
            project, _folderpath, entity_result = try_call(
                resolve_existing_path, args.database, expected="entity"
            )
        ...
        if entity_result is None:
            ...
            if entity_result_class != "database":
                err_exit(
                    "Error: The given object is of class "
                    + entity_result_class
                    + " but an object of class database was expected",
                    3,
                )
...
def remove_developers(args):
    ...
    try:
        if desc["class"] == "app":
            ...
            dxpy.api.global_workflow_remove_developers(
                desc["id"], input_params={"developers": args.developers}
            )
    except:
        err_exit()
...
def uninstall(args):
    ...
    if app_desc:
        ...
        user_data = dxpy.api.user_describe(
            dxpy.whoami(), {"fields": {"appsInstalled": True}}
        )
...
def run_one(args, executable, dest_proj, dest_path, input_json, run_kwargs):
    ...
    if args.confirm and INTERACTIVE_CLI:
        if not prompt_for_yn(
            "Confirm running the executable with this input", default=True
        ):
            parser.exit(0)
    ...
    try:
        ...
        if args.wait:
            ...
            args.confirm
            and INTERACTIVE_CLI
            and not (args.watch or args.ssh)
            and isinstance(dxexecution, dxpy.DXJob)
        ):
        ...
        if (
            run_kwargs.get("detach")
            and os.environ.get("DX_RUN_DETACH") == "1"
            and "detachedJob" in e.msg
        ):
            print(
                "Unable to start detached job in given project. "
...
def run_batch_all_steps(args, executable, dest_proj, dest_path, input_json, run_kwargs):
    if args.wait or args.watch or args.ssh or args.ssh_proxy or args.clone:
        raise Exception(
            "Options {wait, watch, ssh, ssh_proxy, clone} do not work with batch execution"
        )
...
def run_body(
    args, executable, dest_proj, dest_path, preset_inputs=None, input_name_prefix=None
):
    ...
    if args.sys_reqs_from_clone and not isinstance(args.instance_type, str):
        args.instance_type = dict(
            {
                stage: reqs["instanceType"]
                for stage, reqs in list(args.sys_reqs_from_clone.items())
            },
    ...
    if args.sys_reqs_from_clone and not isinstance(args.instance_count, str):
        ...
        args.instance_count = dict(
            {
                fn: reqs["clusterSpec"]["initialInstanceCount"]
                for fn, reqs in list(args.sys_reqs_from_clone.items())
    ...
    if args.instance_count is not None:
        ...
        srd_default = SystemRequirementsDict.from_sys_requirements(
            executable_describe["runSpec"].get("systemRequirements", {}),
            _type="clusterSpec",
        )
    ...
    run_kwargs = {
        ...
        "max_tree_spot_wait_time": normalize_timedelta(args.max_tree_spot_wait_time)
        // 1000
        if args.max_tree_spot_wait_time
        else None,
        "max_job_spot_wait_time": normalize_timedelta(args.max_job_spot_wait_time)
        // 1000
        if args.max_job_spot_wait_time
        else None,
        ...
        if run_kwargs["priority"] in ["low", "normal"]:
            if not args.brief:
                print(
                    fill(
                        BOLD("WARNING")
                        + ": You have requested that jobs be run under "
    ...
    if run_kwargs["priority"] in ["low", "normal"] and not args.brief:
        ...
        def check_for_special_access(access_spec):
            ...
            if (
                access_spec.get("project") in write_perms
                or access_spec.get("allProjects") in write_perms
            ):
    ...
    if not args.brief:
        if isinstance(executable, dxpy.DXWorkflow):
            try:
                dry_run = dxpy.api.workflow_dry_run(
                    executable.get_id(),
                ...
                num_cached_stages = len(
                    [
                        stage
                        for stage in dry_run["stages"]
                        if "parentAnalysis" in stage["execution"]
                        and stage["execution"]["parentAnalysis"] != dry_run["id"]
                    ]
                )
                if num_cached_stages > 0:
                    print(
                        fill(
                            "The following "
                            + str(num_cached_stages)
                            + " stage(s) will reuse results from a previous analysis:"
                        )
                    )
                    for i, stage in enumerate(dry_run["stages"]):
                        if (
                            "parentAnalysis" in stage["execution"]
                            and stage["execution"]["parentAnalysis"] != dry_run["id"]
                        ):
    ...
    if args.batch_tsv is None:
        ...
        run_batch_all_steps(
            args, executable, dest_proj, dest_path, input_json, run_kwargs
        )
...
def print_run_help(executable="", alias=None):
    if executable == "":
        ...
        exec_help = (
            "usage: dx run "
            + executable
            + ("" if alias is None else " --alias " + alias)
        )
        ...
        if is_global_workflow:
            ...
            current_region = dxpy.api.project_describe(
                current_project, input_params={"fields": {"region": True}}
            )["region"]
            ...
            exec_desc = handler.append_underlying_workflow_desc(
                exec_desc, current_region
            )
        ...
        advanced_inputs = (
            exec_details.get("advancedInputs", [])
            ...
            if len(input_spec) == 0:
                ...
                for group, params in list(group_array_by_field(input_spec).items()):
                    ...
                    for param in params:
                        param_string = "\n  "
                        param_string += (
                            UNDERLINE(param.get("label", param["name"])) + ": "
                        )
                        ...
                        if "choices" in param:
                            stanzas.append(
                                format_choices_or_suggestions(
                                    "Choices:", param["choices"], param["class"]
                                )
                            )
                        if helpstring != "":
                            stanzas.append(
                        ...
                        if param.get("suggestions"):
                            stanzas.append(
                                format_choices_or_suggestions(
                                    "Suggestions:", param["suggestions"], param["class"]
                                )
                            )
                        param_string += "\n\n".join(stanzas) + ("\n" if stanzas else "")
...
def print_run_input_help():
    ...
    print(
        fill(
            "There are several ways to specify inputs.  In decreasing order of precedence, they are:"
        )
    )
    print(
        """
...
"""
    ...
    print(
        ...
        + fill(
            "Use the -i/--input flag to specify each input field by "
            + BOLD("name")
            + " and "
            + BOLD("value")
            + ".",
...
"""
    ...
    print(
        fill(
            'The example above runs an app called "myApp" with 3 inputs called num (class int), str (class string), and files (class array:file).  (For this method to work, the app must have an input spec so inputs can be interpreted correctly.)  The same input field can be used multiple times if the input class is an array.',
            initial_indent="  ",
            subsequent_indent="  ",
        )
    )
    print(
        "\n"
        + fill(
            BOLD("Job-based object references")
            + " can also be provided using the <job id>:<output name> syntax:",
            initial_indent="  ",
            subsequent_indent="  ",
        )
    )
...
"""
    ...
    print(
        fill(
            "You can "
            + BOLD("extract an element of an array output")
...
"""
    ...
    print(
        fill(
            "When executing "
            + BOLD("workflows")
            + ", stage inputs can be specified using the <stage key>.<input name>=<value> syntax:",
            initial_indent="  ",
            subsequent_indent="  ",
        )
    )
...
"""
    ...
    print(
        fill(
            "<stage key> may be either the ID of the stage, name of the stage, or the number of the stage in the workflow (0 indicates first stage)"
        )
    )
    ...
    print(
        fill(
            "If the "
            + BOLD("workflow")
            + " has explicit, workflow-level inputs, input values must be passed to these workflow-level input fields using the <workflow input name>=<value> syntax:",
            initial_indent="  ",
            subsequent_indent="  ",
        )
    )
...
"""
    ...
    print(
        fill(
            "JSON input can be used directly using the -j/--input-json or -f/--input-json-file flags.  When running an "
            + BOLD("app")
            + " or "
            + BOLD("applet")
            + ", the keys should be the input field names for the app or applet.  When running a "
            + BOLD("workflow")
            + ', the keys should be the input field names for each stage, prefixed by the stage key and a period, e.g. "my_stage.reads" for the "reads" input of stage "my_stage".',
            initial_indent="  ",
            subsequent_indent="  ",
        )
        + "\n"
...
def run(args):
    ...
    if args.ssh_proxy and not args.ssh:
        err_exit(
            exception=DXCLIError("Option --ssh-proxy cannot be specified without --ssh")
        )
    if args.ssh_proxy:
    ...
    if not args.brief and client_ip is not None:
        print(
            "Detected client IP as '{}'. Setting allowed IP ranges to '{}'. To change the permitted IP addresses use --allow-ssh.".format(
                client_ip, ", ".join(args.allow_ssh)
            )
        )
    ...
    if args.clone is None and args.executable == "":
        err_exit(
            parser_map["run"].format_help()
            + fill(
                "Error: Either the executable must be specified, or --clone must be used to indicate a job or analysis to clone"
            ),
            2,
        )
    ...
    if args.project is not None:
        ...
        dest_proj = resolve_container_id_or_name(
            args.project, is_error=True, multi=False
        )

    if args.folder is not None:
        dest_proj, dest_path, _none = try_call(
            resolve_existing_path, args.folder, expected="folder"
        )
    ...
    if args.stage_output_folder or args.stage_relative_output_folder:
        ...
        for stage, stage_folder in args.stage_output_folder:
            _proj, stage_folder, _none = try_call(
                resolve_existing_path, stage_folder, expected="folder"
            )
        ...
        if is_job_id(args.clone) or is_analysis_id(args.clone):
            ...
            if ":" in args.clone:
                ...
                try:
                    ...
                    if proj_id is not None:
                        ...
                        if is_job_id(execution_name_or_id) or is_analysis_id(
                            execution_name_or_id
                        ):
                            clone_desc = dxpy.api.job_describe(execution_name_or_id)
                        else:
                            iterators.append(
                                dxpy.find_executions(
                                    name=execution_name_or_id,
                                    describe={"io": False},
                                    project=proj_id,
                                )
                            )
            ...
            if clone_desc is None:
                ...
                result_choice = paginate_and_pick(
                    ...
                    (
                        lambda result: get_find_executions_string(
                            result["describe"], has_children=False, single_result=True
                        )
                    ),
                )
                if result_choice == "none found":
                    err_exit(
                        "dx run --clone: No matching execution found. Please use a valid job or analysis name or ID.",
                        3,
                    )
        ...
        if clone_desc["class"] == "job":
            ...
            if args.details is None:
                args.details = {
                    "clonedFrom": {
                        "id": clone_desc["id"],
                        "executable": clone_desc.get(
                            "applet", clone_desc.get("app", "")
                        ),
            ...
            if args.executable != "":
                error_mesg = "Workflow executable (\"{}\") cannot be provided when re-running an analysis with 'dx run --clone'. ".format(
                    args.executable
                )
                error_mesg += "You can instead run 'dx run --clone {}', with other optional CLI arguments, to re-run the previously run analysis.".format(
                    args.clone
                )
                err_exit(
                    exception=DXParserError(error_mesg),
                    expected_exceptions=(DXParserError,),
                )
            # make a temporary workflow
            args.executable = dxpy.api.workflow_new(
                {
                    "project": dest_proj,
    ...
    if args.depends_on and (is_workflow or is_global_workflow):
        err_exit(
            exception=DXParserError(
                "-d/--depends-on cannot be supplied when running workflows."
            ),
            expected_exceptions=(DXParserError,),
        )
    if args.head_job_on_demand and (is_workflow or is_global_workflow):
        err_exit(
            exception=DXParserError(
                "--head-job-on-demand cannot be used when running workflows"
            ),
            expected_exceptions=(DXParserError,),
        )
    ...
    args.region = None
    if is_global_workflow:
        args.region = dxpy.api.project_describe(
            dest_proj, input_params={"fields": {"region": True}}
        )["region"]
    if not args.detach:
    ...
    if args.instance_count:
        if is_workflow or is_global_workflow:
            err_exit(
                exception=DXCLIError("--instance-count is not supported for workflows")
            )
...
def watch(args):
    ...
    level_colors.update(
        {level: GREEN() for level in ("NOTICE", "INFO", "DEBUG", "STDOUT", "METRICS")}
    )

    def check_args_compatibility(incompatible_list):
        for adest, aarg in map(
            lambda arg: arg if isinstance(arg, tuple) else (arg, arg), incompatible_list
        ):
    ...
    if incompatible_args:
        err_exit(
            exception=DXCLIError(
                "Can not specify both '%s' and '%s'" % incompatible_args
            )
        )
    ...
    if args.get_stdout:
        ...
        if args.job_ids:
            args.format = (
                BLUE("{job_name} ({job})") + " {level_color}{level}" + ENDC() + " {msg}"
            )
        else:
            args.format = (
                BLUE("{job_name}") + " {level_color}{level}" + ENDC() + " {msg}"
            )
        if args.timestamps:
        ...
        def msg_callback(message):
            ...
            message["job_name"] = (
                log_client.seen_jobs[message["job"]]["name"]
                if message["job"] in log_client.seen_jobs
                else message["job"]
            )
    ...
    input_params = {
        "numRecentMessages": args.num_recent_messages,
    ...
    if (
        "outputReusedFrom" in job_describe
        and job_describe["outputReusedFrom"] is not None
    ):
        args.jobid = job_describe["outputReusedFrom"]
        if not args.quiet:
            print("Output reused from %s" % args.jobid)

    log_client = DXJobLogStreamClient(
        args.jobid,
        input_params=input_params,
        msg_callback=msg_callback,
        msg_output_format=args.format,
        print_job_info=args.job_info,
    )
    ...
    try:
        if not args.quiet:
            print(
                "Watching job %s%s. Press Ctrl+C to stop watching."
                % (args.jobid, (" and sub-jobs" if args.tree else "")),
                file=sys.stderr,
            )
...
def ssh_config(args):
    ...
    if args.revoke:
        ...
        if os.path.exists(os.path.realpath(key_dest)) and os.path.exists(
            os.path.realpath(pub_key_dest)
        ):
            print(
                BOLD("dx")
                + " is already configured to use the SSH key pair at:\n    {}\n    {}".format(
                    key_dest, pub_key_dest
                )
            )
            if (
                pick(
                    [
                        "Use this SSH key pair",
                        "Select or create another SSH key pair...",
                    ]
                )
                == 1
            ):
        ...
        keys = [
            k
            for k in glob.glob(os.path.join(os.path.expanduser("~/.ssh"), "*.pub"))
        ...
        if choice == 0:
            try:
                subprocess.check_call(
                    ["ssh-keygen", "-f", key_dest] + args.ssh_keygen_args
                )
            except subprocess.CalledProcessError:
                err_exit(
                    "Unable to generate a new SSH key pair",
            ...
            if choice == len(choices) - 1:
                ...
                if os.path.exists(key_src) and os.path.exists(pub_key_src):
                    print(
                        "Using {} and {} as the key pair".format(key_src, pub_key_src)
                    )
                elif (
                    key_src.endswith(".pub")
                    and os.path.exists(key_src[:-4])
                    and os.path.exists(key_src)
                ):
...
def update_pub_key(user_id, pub_key_file):
    ...
    print(
        fill(
            "Your account has been configured for use with SSH. Use "
            + BOLD("dx run")
            + " with the --allow-ssh, "
...
def verify_ssh_config():
    try:
        ...
        msg = (
            RED("Warning:")
            + " Unable to verify configuration of your account for SSH connectivity: {}".format(
                e
            )
...
def ssh(args, ssh_config_verified=False):
    ...
    if not args.no_firewall_update:
        ...
        if not all(ip in job_allow_ssh for ip in args.allow_ssh):
            ...
            sys.stdout.write(
                "Updating allowed IP ranges for SSH to '{}'\n".format(
                    ", ".join(job_allow_ssh)
                )
            )
            dxpy.api.job_update(
                object_id=args.job_id, input_params={"allowSSH": job_allow_ssh}
            )
    ...
    if connected:
        ...
        err_exit(
            msg.format(
                h=host, job_id=args.job_id, cmd=BOLD("dx ssh {}".format(args.job_id))
            ),
            exception=DXCLIError(),
        )
    ...
    ssh_args = [
        "ssh",
        "-i",
        os.path.join(dxpy.config.get_user_conf_dir(), "ssh_id"),
        "-o",
        "HostKeyAlias={}".format(known_host),
        "-o",
    ...
    if args.ssh_proxy:
        ssh_args += [
            "-o",
            "ProxyCommand=nc -X connect -x {proxy} %h %p".format(proxy=args.ssh_proxy),
        ]
    ...
    try:
        ...
        if args.check_running and job_desc["state"] == "running":
            msg = "Job {job_id} is still running. Terminate now?".format(
                job_id=args.job_id
            )
        ...
        tip = (
            "Unable to check the state of {job_id}. Please check it and use "
            + BOLD("dx terminate {job_id}")
...
def upgrade(args):
    if len(args.args) == 0:
        try:
            greeting = dxpy.api.system_greet(
                {"client": "dxclient", "version": "v" + dxpy.TOOLKIT_VERSION}, auth=None
            )
...
def generate_batch_inputs(args):
    ...
    try:
        api_result = dxpy.api.system_generate_batch_inputs(
            {"project": project, "folder": folder, "inputs": input_dict}
        )["batchInputs"]
    except:
        err_exit()
    ...
    eprint(
        "Found {num_success} valid batch IDs matching desired pattern.".format(
    ...
    for i, batch in enumerate(batches):
        ...
        def flatten_batch(b):
            return (
                [b["batchPattern"]]
                + [ival["name"] for iname, ival in sorted(b["inputs"].items())]
                + [
                    project + ":" + ival["ids"][0]
                    for iname, ival in sorted(b["inputs"].items())
                ]
            )
        ...
        with open(batch_fname, write_mode) as csvfile:
            ...
            headers = (
                ["batch ID"]
                + [iname for iname in input_names]
                + [iname + " ID" for iname in input_names]
            )
    ...
    for bi in errors:
        eprint(
            'ERROR processing batch ID matching pattern "{id}"'.format(
                id=bi["batchPattern"]
            )
        )
        ...
        if input_names != input_names_i:
            ...
            eprint(
                "        Required input names: {required}".format(
                    required=", ".join(input_names)
                )
            )
            eprint(
                "        Matched input names: {matched}".format(
                    matched=", ".join(input_names_i)
                )
            )
        for input_name, matches in list(bi["inputs"].items()):
            if len(matches["ids"]) > 1:
                eprint(
                    "Input {iname} is associated with a file name that matches multiple IDs:".format(
                        iname=input_name
                    )
                )
                eprint(
                    "    {fname} => {ids}".format(
                        fname=matches["name"], ids=", ".join(matches["ids"])
                    )
                )
        eprint("")

    eprint(
        "CREATED {num_batches} batch files each with at most {max} batch IDs.".format(
            num_batches=len(batches), max=MAX_BATCH_SIZE
        )
    )
    if len(errors) > 0:
        err_exit(
            "ERROR SUMMARY: Found {num_errors} batch IDs with incomplete or ambiguous results.  Details above.".format(
...
def publish(args):
    desc = try_call(
        resolve_global_executable, args.executable, is_version_required=True
    )

    try:
        if desc["class"] == "app":
            dxpy.api.app_publish(
                desc["id"], input_params={"makeDefault": args.make_default}
            )
        else:
            dxpy.api.global_workflow_publish(
                desc["id"], input_params={"makeDefault": args.make_default}
            )
        ...
        if desc["authorizedUsers"]:
            eprint(
                "It is now available to the authorized users: {}".format(
                    ", ".join(desc["authorizedUsers"])
                )
            )
...
def archive(args):
    def send_archive_request(target_project, request_input, request_func):
        ...
        try:
            ...
            err_exit(
                "Failed request: {}. {}".format(request_input, format_exception(e)),
                code=3,
            )
        return res
    ...
    def get_valid_archival_input(args, target_files, target_folder, target_project):
        ...
        if target_files:
            ...
            err_exit(
                "No input file/folder is found in project {}".format(target_project),
                code=3,
            )
    ...
    def get_archival_paths(args):
        ...
        paths = [
            split_unescaped(":", path, include_empty_strings=True) for path in args.path
        ]
        ...
        for p in paths:
            if len(p) > 2:
                err_exit(
                    "Path '{}' is invalid. Please check the inputs or check --help for example inputs.".format(
                        ":".join(p)
                    ),
                    code=3,
                )
            ...
            obj = p[-1]
            if obj[-1] == "/":
                folder, entity_name = clean_folder_path(
        ...
        for proj in possible_projects:
            ...
            # is "": use current project
                ...
                if not dxpy.PROJECT_CONTEXT_ID:
                    err_exit(
                        "Cannot find current project. Please check the environment.",
                        code=3,
                    )
            ...
            # name is given
                ...
                if project_results:
                    choice = pick(
                        [
                            "{} ({})".format(result["describe"]["name"], result["id"])
            ...
            if target_project and proj != target_project:
                err_exit(
                    "All paths must refer to files/folder in a single project, but two project ids: '{}' and '{}' are given. ".format(
                        target_project, proj
                    ),
                    code=3,
                )
        ...
        if not target_project:
            err_exit(
                "No target project has been set. Please check the input or check your permission to the given project.",
                code=3,
            )
        if len(possible_folder) > 1:
            err_exit(
                "Only one folder is allowed for each request. Please check the inputs or check --help for example inputs.".format(
                    p
                ),
                code=3,
            )
        if possible_folder and possible_files:
            err_exit(
                "Expecting either a single folder or a list of files for each API request",
                code=3,
            )
        ...
        if possible_folder:
            ...
            for fp in possible_files:
                ...
                # is folderpath/filename
                    ...
                    folderpath, filename = clean_folder_path(
                        ("" if obj.startswith("/") else "/") + fp
                    )
                    try:
                        file_results = list(
                            dxpy.find_data_objects(
                        ...
                        err_exit(
                            "Input '{}' is not found as a file in project '{}'".format(
                                fp, target_project
                            ),
                            code=3,
                        )
                    ...
                    if not args.all:
                        choice = pick(
                            [
                                "{} ({})".format(
                                    result["describe"]["name"], result["id"]
                                )
    ...
    request_mode, request_func, request_input = get_valid_archival_input(
        args, target_files, target_folder, target_project
    )
    ...
    if args.confirm and INTERACTIVE_CLI:
        if request_mode == "archival":
            if target_files:
                ...
                print(
                    "Will tag {} file(s) for archival in {}".format(
                        counts, target_project
                    )
                )
            else:
                print(
                    "Will tag file(s) for archival in folder {}:{} {}recursively".format(
                        target_project,
                        target_folder,
                        "non-" if not args.recurse else "",
                    )
                )
        elif request_mode == "unarchival":
            dryrun_request_input = copy.deepcopy(request_input)
            dryrun_request_input.update(dryRun=True)
            dryrun_res = send_archive_request(
                target_project, dryrun_request_input, request_func
            )
            print(
                "Will tag {} file(s) for unarchival in {}, totalling {} GB, costing ${}".format(
                    dryrun_res["files"],
                    target_project,
                    dryrun_res["size"],
                    dryrun_res["cost"] / 1000,
                )
            )
    ...
    if not args.quiet:
        ...
        if request_mode == "archival":
            print(
                "Tagged {} file(s) for archival in {}".format(
                    res["count"], target_project
                )
            )
        elif request_mode == "unarchival":
            print(
                "Tagged {} file(s) for unarchival, totalling {} GB, costing ${}".format(
                    res["files"], res["size"], res["cost"] / 1000
                )
            )
        print()
...
def print_help(args):
    if args.command_or_category is None:
        ...
        print(
            "dx "
            + args.command_or_category
            + ": "
            + parser_categories[args.command_or_category]["desc"].lstrip()
        )
        print("\nCommands:\n")
        for cmd in parser_categories[args.command_or_category]["cmds"]:
            print(
                ...
                + cmd[0]
                + " " * (18 - len(cmd[0]))
                + fill(cmd[1], width_adjustment=-20, subsequent_indent=" " * 20)
            )
        ...
        err_exit(
            "Unrecognized command and subcommand combination: "
            + args.command_or_category
            + " "
            + args.subcommand,
            3,
...
class PrintCategoryHelp(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        print(
            fill(
                "List only the {et} that belong to a particular category by providing a category name.".format(
                    et=exectype
                )
            )
        )
...
class DXNewUserOrgArgsAction(argparse.Action):
    ...
    def __init__(
        self, option_strings, dest, required=False, default=None, nargs=None, **kwargs
    ):
...
class DXArgumentParser(argparse.ArgumentParser):
    def _print_message(self, message, file=None):
        if message:
            if (
                message.startswith("usage: dx [-h] [--version] command")
                and "dx: error: argument command: invalid choice:" in message
            ):
    ...
    def error(self, message):
        ...
        self.exit(
            2,
            "{help}\n{prog}: error: {msg}\n".format(
                help=self.format_help(), prog=self.prog, msg=message
            ),
        )


def register_parser(
    parser, subparsers_action=None, categories=("other",), add_help=True
):
...
parser = DXArgumentParser(
    description=DNANEXUS_LOGO()
    + " Command-Line Client, API v%s, client v%s"
    % (dxpy.API_VERSION, dxpy.TOOLKIT_VERSION)
    + "\n\n"
    + fill(
        "dx is a command-line client for interacting with the DNAnexus platform.  You can log in, navigate, upload, organize and share your data, launch analyses, and more.  For a quick tour of what the tool can do, see"
    )
    ...
    + "\n\n"
    + fill(
        "dx exits with exit code 3 if invalid input is provided or an invalid operation is requested, and exit code 1 if an internal error is encountered.  The latter usually indicate bugs in dx; please report them at"
    )
...
host_action = parser_login.add_argument(
    "--host", help="Log into the given auth server host (port must also be given)"
)
port_action = parser_login.add_argument(
    "--port",
    type=int,
    help="Log into the given auth server port (host must also be given)",
)
protocol_action = parser_login.add_argument(
    "--protocol",
...
parser_login.add_argument(
    "--noprojects",
    dest="projects",
    help="Do not print available projects",
    action="store_false",
)
parser_login.add_argument(
    "--save",
    help="Save token and other environment variables for future sessions",
    action="store_true",
)
parser_login.add_argument(
    "--timeout",
...
parser_login.add_argument(
    "--staging", nargs=0, help=argparse.SUPPRESS, action=SetStagingEnv
)
...
parser_logout.add_argument(
    "--host", help="Log out of the given auth server host (port must also be given)"
)
parser_logout.add_argument(
    "--port",
    type=int,
    help="Log out of the given auth server port (host must also be given)",
)
parser_logout.add_argument(
    "--protocol",
    help="Used in conjunction with host and port arguments, gives the protocol to use when contacting auth server",
...
parser_exit = subparsers.add_parser(
    "exit",
    help="Exit out of the interactive shell",
    description="Exit out of the interactive shell",
    prog="dx exit",
)
...
parser_whoami.add_argument(
    "--id",
    help="Print user ID instead of username",
    action="store_true",
    dest="user_id",
)
...
parser_env = subparsers.add_parser(
    "env",
    help="Print all environment variables in use",
    description=fill(
        "Prints all environment variables in use as they have been resolved from environment variables and configuration files.  For more details, see"
    )
...
parser_env.add_argument(
    "--bash",
    help=fill(
        "Prints a list of bash commands to export the environment variables",
        width_adjustment=-14,
    ),
    action="store_true",
)
parser_env.add_argument(
    "--dx-flags",
    help=fill(
        "Prints the dx options to override the environment variables",
        width_adjustment=-14,
    ),
    action="store_true",
)
...
parser_setenv.add_argument(
    "--save",
    help="Save settings for future sessions.  Only one set of settings can be saved at a time.  Always set to true if login is run in a non-interactive session",
    action="store_true",
)
parser_setenv.add_argument(
    "--current",
    help="Do not prompt for new values and just save current settings for future sessions.  Overrides --save to be true.",
    action="store_true",
)
...
# clearenv
#####################################
parser_clearenv = subparsers.add_parser(
    "clearenv",
    help="Clears all environment variables set by dx",
    description="Clears all environment variables set by dx.  More specifically, it removes local state stored in ~/.dnanexus_config/environment.  Does not affect the environment variables currently set in your shell.",
    prog="dx clearenv",
)
parser_clearenv.add_argument(
    "--reset",
    help="Reset dx environment variables to empty values. Use this to avoid interference between multiple dx sessions when using shell environment variables.",
    action="store_true",
)
...
parser_invite.add_argument(
    "project", help="Project to invite the invitee to", default=":", nargs="?"
)
parser_invite.add_argument(
    "level",
    help="Permissions level the new member should have",
    choices=["VIEW", "UPLOAD", "CONTRIBUTE", "ADMINISTER"],
    default="VIEW",
    nargs="?",
)
parser_invite.add_argument(
    ...
    dest="send_email",
    action="store_false",
    help="Disable email notifications to invitee",
)
...
# uninvite
#####################################
parser_uninvite = subparsers.add_parser(
    "uninvite",
...
parser_uninvite.add_argument(
    "project", help="Project to revoke permissions from", default=":", nargs="?"
)
...
ls_output_args.add_argument(
    "-l", "--long", dest="verbose", help='Alias for "verbose"', action="store_true"
)
...
ls_path_action = parser_ls.add_argument(
    "path",
    help="Folder (possibly in another project) to list the contents of, default is the current directory in the current project.  Syntax: projectID:/folder/path",
    nargs="?",
    default=".",
)
...
tree_path_action = parser_tree.add_argument(
    "path",
    help="Folder (possibly in another project) to list the contents of, default is the current directory in the current project.  Syntax: projectID:/folder/path",
    nargs="?",
    default=".",
...
select_project_action = parser_select.add_argument(
    "project",
    help="Name or ID of a project to switch to; if not provided a list will be provided for you",
    nargs="?",
    default=None,
)
select_project_action.completer = DXPathCompleter(
    expected="project", include_current_proj=False
)
parser_select.add_argument(
    "--name", help="Name of the project (wildcard patterns supported)"
)
parser_select.add_argument(
    "--level",
    choices=["VIEW", "UPLOAD", "CONTRIBUTE", "ADMINISTER"],
...
cd_path_action = parser_cd.add_argument(
    "path",
    nargs="?",
    default="/",
...
parser_cp = subparsers.add_parser(
    "cp",
    help="Copy objects and/or folders between different projects",
    formatter_class=argparse.RawTextHelpFormatter,
    description=fill(
        "Copy objects and/or folders between different projects.  Folders will automatically be copied recursively.  To specify which project to use as a source or destination, prepend the path or ID of the object/folder with the project ID or name and a colon."
    )
    + """
...
EXAMPLES
  ...
  """
    + fill(
        'The first example copies a file in a project called "FirstProj" to the current directory of the current project.  The second example copies the object named "reads.fq.gz" in the current directory to the folder /folder/path in the project with ID "project-B0VK6F6gpqG6z7JGkbqQ000Q", and finally renaming it to "newname.fq.gz".',
        width_adjustment=-2,
        subsequent_indent="  ",
    )
    + """
...
cp_sources_action = parser_cp.add_argument(
    "sources", help="Objects and/or folder names to copy", metavar="source", nargs="+"
)
cp_sources_action.completer = DXPathCompleter()
parser_cp.add_argument(
    "destination",
    help=fill(
        "Folder into which to copy the sources or new pathname (if only one source is provided).  Must be in a different project/container than all source paths.",
        width_adjustment=-15,
    ),
)
...
parser_mv = subparsers.add_parser(
    "mv",
    help="Move or rename objects and/or folders inside a project",
    formatter_class=argparse.RawTextHelpFormatter,
    description=fill(
        "Move or rename data objects and/or folders inside a single project.  To copy data between different projects, use 'dx cp' instead."
    ),
    prog="dx mv",
    parents=[env_args, all_arg],
)
mv_sources_action = parser_mv.add_argument(
    "sources", help="Objects and/or folder names to move", metavar="source", nargs="+"
)
mv_sources_action.completer = DXPathCompleter()
parser_mv.add_argument(
    "destination",
    help=fill(
        "Folder into which to move the sources or new pathname (if only one source is provided).  Must be in the same project/container as all source paths.",
        width_adjustment=-15,
    ),
)
...
mkdir_paths_action = parser_mkdir.add_argument(
    "paths", help="Paths to folders to create", metavar="path", nargs="+"
)
...
rmdir_paths_action = parser_rmdir.add_argument(
    "paths", help="Paths to folders to remove", metavar="path", nargs="+"
)
...
rm_paths_action = parser_rm.add_argument(
    "paths", help="Paths to remove", metavar="path", nargs="+"
)
rm_paths_action.completer = DXPathCompleter()
parser_rm.add_argument(
    "-r", "--recursive", help="Recurse into a directory", action="store_true"
)
parser_rm.add_argument(
    "-f", "--force", help="Force removal of files", action="store_true"
)
...
parser_describe = subparsers.add_parser(
    "describe",
    help="Describe a remote object",
    description=fill(
        'Describe a DNAnexus entity.  Use this command to describe data objects by name or ID, jobs, apps, users, organizations, etc.  If using the "--json" flag, it will thrown an error if more than one match is found (but if you would like a JSON array of the describe hashes of all matches, then provide the "--multi" flag).  Otherwise, it will always display all results it finds.'
    )
    + "\n\nNOTES:\n\n- "
    + fill(
        "The project found in the path is used as a HINT when you are using an object ID; you may still get a result if you have access to a copy of the object in some other project, but if it exists in the specified project, its description will be returned."
    )
    + "\n\n- "
    + fill(
        "When describing apps or applets, options marked as advanced inputs will be hidden unless --verbose is provided"
    ),
    formatter_class=argparse.RawTextHelpFormatter,
...
parser_describe.add_argument(
    "--details", help="Include details of data objects", action="store_true"
)
parser_describe.add_argument(
    "--verbose", help="Include additional metadata", action="store_true"
)
parser_describe.add_argument(
    "--name", help="Only print the matching names, one per line", action="store_true"
)
parser_describe.add_argument(
    "--multi",
    help=fill(
        "If the flag --json is also provided, then returns a JSON array of describe hashes of all matching results",
        width_adjustment=-24,
    ),
    action="store_true",
)
describe_path_action = parser_describe.add_argument(
    "path",
    help=fill(
        "Object ID or path to an object (possibly in another project) to describe.",
        width_adjustment=-24,
    ),
)
...
upload_filename_action = parser_upload.add_argument(
    "filename",
    nargs="+",
...
parser_upload.add_argument(
    "-o", "--output", help=argparse.SUPPRESS
)  # deprecated; equivalent to --path/--destination
parser_upload.add_argument(
    "--path",
    "--destination",
    help=fill(
        "DNAnexus path to upload file(s) to (default uses current project and folder if not provided)",
        width_adjustment=-24,
    ),
    nargs="?",
)
parser_upload.add_argument(
    "-r", "--recursive", help="Upload directories recursively", action="store_true"
)
parser_upload.add_argument(
    "--wait", help="Wait until the file has finished closing", action="store_true"
)
parser_upload.add_argument(
    "--no-progress",
...
parser_upload.add_argument(
    "--buffer-size",
    help="Set the write buffer size (in bytes)",
    dest="write_buffer_size",
)
parser_upload.add_argument(
    "--singlethread",
    help="Enable singlethreaded uploading",
    dest="singlethread",
    action="store_true",
)
...
parser_download_paths_arg = parser_download.add_argument(
    "paths",
    help="Data object ID or name, or folder to download",
    nargs="+",
    metavar="path",
)
...
parser_download.add_argument(
    "-o",
    "--output",
    help='Local filename or directory to be used ("-" indicates stdout output); if not supplied or a directory is given, the object\'s name on the platform will be used, along with any applicable extensions',
)
parser_download.add_argument(
    "-f",
    "--overwrite",
    help="Resume an interupted download if the local and remote file signatures match.  If the signatures do not match the local file will be overwritten.",
    action="store_true",
)
parser_download.add_argument(
    "-r", "--recursive", help="Download folders recursively", action="store_true"
)
parser_download.add_argument(
    "-a",
    "--all",
    help="If multiple objects match the input, download all of them",
    action="store_true",
)
parser_download.add_argument(
    "--no-progress",
...
parser_download.add_argument(
    "--lightweight",
    help="Skip some validation steps to make fewer API calls",
    action="store_true",
)
parser_download.add_argument(
    "--symlink-max-tries",
    help="Set maximum number of tries for downloading symlinked files using aria2c",
    type=positive_integer,
    default=15,
)
parser_download.add_argument(
    "--unicode",
    help="Display the characters as text/unicode when writing to stdout",
    dest="unicode_text",
    action="store_true",
)
...
# make_download_url
#####################################
parser_make_download_url = subparsers.add_parser(
    "make_download_url",
    help="Create a file download link for sharing",
...
path_action = parser_make_download_url.add_argument(
    "path",
    help="Project-qualified data object ID or name, e.g. project-xxxx:file-yyyy, or project-xxxx:/path/to/file.txt",
)
path_action.completer = DXPathCompleter(classes=["file"])
parser_make_download_url.add_argument(
    "--duration",
    help="Time for which the URL will remain valid (in seconds, or use suffix s, m, h, d, w, M, y). Default: 1 day",
)
parser_make_download_url.add_argument(
    "--filename",
    help="Name that the server will instruct the client to save the file as (default is the filename)",
)
...
parser_cat = subparsers.add_parser(
    "cat", help="Print file(s) to stdout", prog="dx cat", parents=[env_args]
)
cat_path_action = parser_cat.add_argument(
    "path", help="File ID or name(s) to print to stdout", nargs="+"
)
...
build_parser = subparsers.add_parser(
    "build",
    help="Create a new applet/app, or a workflow",
    description="Build an applet, app, or workflow object from a local source directory or an app from an existing applet in the platform. You can use "
    + BOLD("dx-app-wizard")
    + " to generate a skeleton directory of an app/applet with the necessary files.",
    prog="dx build",
    parents=[env_args, stdout_args],
)

app_and_globalworkflow_options = build_parser.add_argument_group(
    "Options for creating apps or globalworkflows",
    "(Only valid when --app/--create-app/--globalworkflow/--create-globalworkflow is specified)",
)
applet_and_workflow_options = build_parser.add_argument_group(
    "Options for creating applets or workflows",
    "(Only valid when --app/--create-app/--globalworkflow/--create-globalworkflow is NOT specified)",
)
nextflow_options = build_parser.add_argument_group(
    "Options for creating Nextflow applets", "(Only valid when --nextflow is specified)"
)

# COMMON OPTIONS
build_parser.add_argument(
    "--ensure-upload",
    help="If specified, will bypass computing checksum of "
...
build_parser.add_argument(
    "--force-symlinks",
    help="If specified, will not attempt to dereference "
...
src_dir_action = build_parser.add_argument(
    "src_dir",
    help="Source directory that contains dxapp.json, dxworkflow.json or *.nf (for --nextflow option). (default: current directory)",
    nargs="?",
)
...
build_parser.add_argument(
    "--globalworkflow",
    "--create-globalworkflow",
    help="Create a global workflow.",
    action="store_const",
    dest="mode",
    const="globalworkflow",
)

applet_and_workflow_options.add_argument(
    "-d",
    "--destination",
    help="Specifies the destination project, destination folder, and/or name for the applet, in the form [PROJECT_NAME_OR_ID:][/[FOLDER/][NAME]]. Overrides the project, folder, and name fields of the dxapp.json or dxworkflow.json, if they were supplied.",
    default=".",
)
...
build_parser.add_argument(
    "--dry-run",
    "-n",
    help="Do not create an app(let): only perform local checks and compilation steps, and show the spec of the app(let) that would have been created.",
    action="store_true",
    dest="dry_run",
)
build_parser.add_argument(
    "--no-dry-run", help=argparse.SUPPRESS, action="store_false", dest="dry_run"
)

# --[no-]publish
app_and_globalworkflow_options.set_defaults(publish=False)
app_and_globalworkflow_options.add_argument(
    "--publish",
    help="Publish the resulting app/globalworkflow and make it the default.",
    action="store_true",
    dest="publish",
)
app_and_globalworkflow_options.add_argument(
    "--no-publish", help=argparse.SUPPRESS, action="store_false", dest="publish"
)
app_and_globalworkflow_options.add_argument(
    "--from",
    help="ID or path of the source applet/workflow to create an app/globalworkflow from. Source directory src_dir cannot be given when using this option",
    dest="_from",
).completer = DXPathCompleter(classes=["applet", "workflow"])
...
build_parser.add_argument(
    "--remote",
    help="Build the app remotely by uploading the source directory to the DNAnexus Platform and building it there. This option is useful if you would otherwise need to cross-compile the app(let) to target the Execution Environment.",
    action="store_true",
    dest="remote",
)
build_parser.add_argument(
    "--no-watch",
    help="Don't watch the real-time logs of the remote builder (this option is only applicable if --remote or --repository is specified).",
    action="store_false",
    dest="watch",
)
build_parser.add_argument(
    "--no-remote", help=argparse.SUPPRESS, action="store_false", dest="remote"
)

applet_and_workflow_options.add_argument(
    "-f",
    "--overwrite",
    help="Remove existing applet(s) of the same name in the destination folder. This option is not yet supported for workflows.",
    action="store_true",
    default=False,
)
applet_and_workflow_options.add_argument(
    "-a",
    "--archive",
    help="Archive existing applet(s) of the same name in the destination folder. This option is not yet supported for workflows.",
    action="store_true",
    default=False,
)
build_parser.add_argument(
    "-v",
    "--version",
    help="Override the version number supplied in the manifest. This option needs to be specified when using --from option.",
    default=None,
...
app_and_globalworkflow_options.add_argument(
    "-b",
    "--bill-to",
    help="Entity (of the form user-NAME or org-ORGNAME) to bill for the app/globalworkflow.",
    default=None,
    dest="bill_to",
    metavar="USER_OR_ORG",
)
...
build_parser.add_argument(
    "--check-syntax", help=argparse.SUPPRESS, action="store_true", dest="check_syntax"
)
build_parser.add_argument(
    "--no-check-syntax",
    help="Warn but do not fail when syntax problems are found (default is to fail on such errors).",
    action="store_false",
    dest="check_syntax",
)
...
app_and_globalworkflow_options.add_argument(
    ...
    help=argparse.SUPPRESS,
    action="store_true",
    dest="version_autonumbering",
)
app_and_globalworkflow_options.add_argument(
    "--no-version-autonumbering",
    help="Only attempt to create the version number supplied in the manifest (that is, do not try to create an autonumbered version such as 1.2.3+git.ab1b1c1d if 1.2.3 already exists and is published).",
    action="store_false",
    dest="version_autonumbering",
)
# --[no-]update
...
app_and_globalworkflow_options.add_argument(
    "--update", help=argparse.SUPPRESS, action="store_true", dest="update"
)
app_and_globalworkflow_options.add_argument(
    "--no-update",
    help="Never update an existing unpublished app/globalworkflow in place.",
    action="store_false",
    dest="update",
)
# --[no-]dx-toolkit-autodep
build_parser.add_argument(
    "--dx-toolkit-legacy-git-autodep",
    help=argparse.SUPPRESS,
    action="store_const",
    dest="dx_toolkit_autodep",
    const="git",
)
build_parser.add_argument(
    "--dx-toolkit-stable-autodep",
    help=argparse.SUPPRESS,
    action="store_const",
    dest="dx_toolkit_autodep",
    const="stable",
...
build_parser.add_argument(
    "--no-parallel-build",
    help="Build with " + BOLD("make") + " instead of " + BOLD("make -jN") + ".",
    action="store_false",
    dest="parallel_build",
)
...
app_and_globalworkflow_options.add_argument(
    ...
    help="When building an app in a single region, build its applet in the current project instead of a temporary project.",
    action="store_false",
    dest="use_temp_build_project",
)
...
app_and_globalworkflow_options.add_argument(
    ...
    "--yes",
    dest="confirm",
    help="Do not ask for confirmation for potentially dangerous operations",
    action="store_false",
)
...
build_parser.add_argument(
    "--json", help=argparse.SUPPRESS, action="store_true", dest="json"
)
build_parser.add_argument(
    "--no-json", help=argparse.SUPPRESS, action="store_false", dest="json"
)
build_parser.add_argument(
    "--extra-args",
    help="Arguments (in JSON format) to pass to the /applet/new API method, overriding all other settings",
)
build_parser.add_argument(
    "--run",
    help="Run the app or applet after building it (options following this are passed to "
    + BOLD("dx run")
    + "; run at high priority by default).",
...
app_and_globalworkflow_options.add_argument(
    "--region",
    action="append",
    help="Enable the app/globalworkflow in this region. This flag can be specified multiple times to enable the app/globalworkflow in multiple regions. If --region is not specified, then the enabled region(s) will be determined by 'regionalOptions' in dxapp.json, or the project context.",
)
...
build_parser.add_argument(
    "--keep-open",
    help=fill(
        "Do not close workflow after building it. Cannot be used when building apps, applets or global workflows.",
        width_adjustment=-24,
    ),
    action="store_true",
)
...
build_parser.add_argument(
    "--nextflow",
    help=fill("Build Nextflow applet.", width_adjustment=-24),
    action="store_true",
)

# --profile
nextflow_options.add_argument(
    "--profile",
    help=fill("Default profile for the Nextflow pipeline.", width_adjustment=-24),
    dest="profile",
)
...
nextflow_options.add_argument(
    "--git-credentials",
    help=fill(
        "Git credentials used to access Nextflow pipelines from private Git repositories. "
...
).completer = DXPathCompleter(classes=["file"])
...
parser_build_asset = subparsers.add_parser(
    ...
    formatter_class=argparse.RawTextHelpFormatter,
    description=fill(
        "Build an asset from a local source directory. The directory must have a file called "
...
parser_build_asset.add_argument(
    "src_dir", help="Asset source directory (default: current directory)", nargs="?"
)
parser_build_asset.add_argument(
    "-d",
    "--destination",
    help=fill(
        "Specifies the destination project and destination folder for the asset,"
...
parser_build_asset.add_argument(
    "--json",
    help=fill("Show ID of resulting asset bundle in JSON format"),
    action="store_true",
    dest="json",
)
parser_build_asset.add_argument(
    "--no-watch",
    help=fill("Don't watch the real-time logs of the asset-builder job."),
    action="store_false",
    dest="watch",
)
parser_build_asset.add_argument(
    "--priority", choices=["normal", "high"], help=argparse.SUPPRESS
)
...
parser_add = subparsers.add_parser(
    "add",
    help="Add one or more items to a list",
...
parser_add_users = subparsers_add.add_parser(
    "users",
    help="Add authorized users for an app",
...
parser_add_users.add_argument(
    "app", help="Name or ID of an app"
).completer = DXAppCompleter(installed=True)
parser_add_users.add_argument(
    "users",
    metavar="authorizedUser",
...
parser_add_developers = subparsers_add.add_parser(
    "developers",
    help="Add developers for an app",
...
parser_add_developers.add_argument(
    "app", help="Name or ID of an app"
).completer = DXAppCompleter(installed=True)
parser_add_developers.add_argument(
    "developers",
...
register_parser(
    parser_add_developers, subparsers_action=subparsers_add, categories="exec"
)

parser_add_stage = subparsers_add.add_parser(
    "stage",
    help="Add a stage to a workflow",
    description="Add a stage to a workflow.  Default inputs for the stage can also be set at the same time.",
    parents=[exec_input_args, stdout_args, env_args, instance_type_arg],
    prog="dx add stage",
)
parser_add_stage.add_argument(
    "workflow", help="Name or ID of a workflow"
).completer = DXPathCompleter(classes=["workflow"])
parser_add_stage.add_argument(
    "executable", help="Name or ID of an executable to add as a stage in the workflow"
).completer = MultiCompleter([DXAppCompleter(), DXPathCompleter(classes=["applet"])])
parser_add_stage.add_argument(
    "--alias",
    "--version",
    "--tag",
...
add_stage_folder_args.add_argument(
    "--output-folder",
    help="Path to the output folder for the stage (interpreted as an absolute path)",
)
add_stage_folder_args.add_argument(
    "--relative-output-folder",
    help="A relative folder path for the stage (interpreted as relative to the workflow's output folder)",
)
parser_add_stage.set_defaults(func=workflow_cli.add_stage)
register_parser(
    parser_add_stage, subparsers_action=subparsers_add, categories="workflow"
)

parser_add_member = subparsers_add.add_parser(
    "member",
    help="Grant a user membership to an org",
    description="Grant a user membership to an org",
    prog="dx add member",
...
parser_add_member.add_argument(
    "--level",
    required=True,
    choices=["ADMIN", "MEMBER"],
    help="Org membership level that will be granted to the specified user",
)
parser_add_member.add_argument(
    "--allow-billable-activities",
    default=False,
    action="store_true",
    help='Grant the specified user "allowBillableActivities" in the org',
)
parser_add_member.add_argument(
    "--no-app-access",
    default=True,
    action="store_false",
    dest="app_access",
    help='Disable "appAccess" for the specified user in the org',
)
parser_add_member.add_argument(
    "--project-access",
    choices=["ADMINISTER", "CONTRIBUTE", "UPLOAD", "VIEW", "NONE"],
    default="CONTRIBUTE",
    help="The default implicit maximum permission the specified user will receive to projects explicitly shared with the org; default CONTRIBUTE",
)
parser_add_member.add_argument(
    "--no-email",
    default=False,
    action="store_true",
    help="Disable org invitation email notification to the specified user",
)
...
parser_list = subparsers.add_parser(
    "list",
    help="Print the members of a list",
...
parser_list_users = subparsers_list.add_parser(
    "users",
    help="List authorized users for an app",
...
parser_list_developers = subparsers_list.add_parser(
    "developers",
    help="List developers for an app",
...
parser_list_developers.add_argument(
    "app", help="Name or ID of an app"
).completer = DXAppCompleter(installed=True)
parser_list_developers.set_defaults(func=list_developers)
register_parser(
    parser_list_developers, subparsers_action=subparsers_list, categories="exec"
)

parser_list_stages = subparsers_list.add_parser(
    "stages",
    help="List the stages in a workflow",
    description="List the stages in a workflow.",
    parents=[env_args],
    prog="dx list stages",
)
parser_list_stages.add_argument(
    "workflow", help="Name or ID of a workflow"
).completer = DXPathCompleter(classes=["workflow"])
parser_list_stages.set_defaults(func=workflow_cli.list_stages)
register_parser(
    parser_list_stages, subparsers_action=subparsers_list, categories="workflow"
)
...
parser_list_database = subparsers_list.add_parser(
    ...
    + "\n\n\t"
    + fill(
        '"dx list database files" lists database files associated with a specific database.'
    )
    + "\n\n\t"
...
subparsers_list_database = parser_list_database.add_subparsers(
...
parser_list_database_files.add_argument(
    "database", help="Data object ID or path of the database."
)
parser_list_database_files.add_argument(
    "--folder",
    default="/",
    help='Name of folder (directory) in which to start searching for database files. This will typically match the name of the table whose files are of interest. The default value is "/" which will start the search at the root folder of the database.',
)
parser_list_database_files.add_argument(
    "--recurse",
    default=False,
    help="Look for files recursively down the directory structure. Otherwise, by default, only look on one level.",
    action="store_true",
)
parser_list_database_files.add_argument(
    "--csv",
    default=False,
    help="Write output as comma delimited fields, suitable as CSV format.",
    action="store_true",
)
parser_list_database_files.add_argument(
    "--timeout",
    default=120,
    help="Number of seconds to wait before aborting the request. If omitted, default timeout is 120 seconds.",
    type=int,
)
parser_list_database_files.set_defaults(func=list_database_files)
register_parser(
    parser_list_database_files,
    subparsers_action=subparsers_list_database,
    categories="data",
)
...
parser_remove = subparsers.add_parser(
    "remove",
    help="Remove one or more items to a list",
...
parser_remove_users = subparsers_remove.add_parser(
    "users",
    help="Remove authorized users for an app",
...
parser_remove_users.add_argument(
    "app", help="Name or ID of an app"
).completer = DXAppCompleter(installed=True)
parser_remove_users.add_argument(
    "users",
...
register_parser(
    parser_remove_users, subparsers_action=subparsers_remove, categories="exec"
)

parser_remove_developers = subparsers_remove.add_parser(
    "developers",
    help="Remove developers for an app",
...
parser_remove_developers.add_argument(
    "app", help="Name or ID of an app"
).completer = DXAppCompleter(installed=True)
parser_remove_developers.add_argument(
    "developers", metavar="developer", help="One or more users to remove", nargs="+"
)
parser_remove_developers.set_defaults(func=remove_developers)
register_parser(
    parser_remove_developers, subparsers_action=subparsers_remove, categories="exec"
)

parser_remove_stage = subparsers_remove.add_parser(
    "stage",
    help="Remove a stage from a workflow",
...
parser_remove_stage.add_argument(
    "workflow", help="Name or ID of a workflow"
).completer = DXPathCompleter(classes=["workflow"])
parser_remove_stage.add_argument(
    "stage", help="Stage (index or ID) of the workflow to remove"
)
parser_remove_stage.set_defaults(func=workflow_cli.remove_stage)
register_parser(
    parser_remove_stage, subparsers_action=subparsers_remove, categories="workflow"
)

parser_remove_member = subparsers_remove.add_parser(
    "member",
    help="Revoke the org membership of a user",
    description="Revoke the org membership of a user",
    prog="dx remove member",
    parents=[stdout_args, env_args],
)
...
parser_remove_member.add_argument(
    ...
    default=True,
    action="store_false",
    dest="revoke_project_permissions",
    help="Disable revocation of explicit project permissions of the specified user to projects billed to the org; implicit project permissions (i.e. those granted to the specified user via his membership in this org) will always be revoked",
)
parser_remove_member.add_argument(
    "--keep-explicit-app-permissions",
    default=True,
    action="store_false",
    dest="revoke_app_permissions",
    help="Disable revocation of explicit app developer and user permissions of the specified user to apps billed to the org; implicit app permissions (i.e. those granted to the specified user via his membership in this org) will always be revoked",
)
parser_remove_member.add_argument(
    "-y",
    "--yes",
...
register_parser(
    parser_remove_member, subparsers_action=subparsers_remove, categories="org"
)
...
parser_update = subparsers.add_parser(
    "update",
    help="Update certain types of metadata",
    description="""
...
parser_update_org = subparsers_update.add_parser(
    "org",
...
parser_update_org.add_argument(
    "--member-list-visibility",
    help="New org membership level that is required to be able to view the membership level and/or permissions of any other member in the specified org (corresponds to the memberListVisibility org policy)",
    choices=["ADMIN", "MEMBER", "PUBLIC"],
)
parser_update_org.add_argument(
    "--project-transfer-ability",
    help="New org membership level that is required to be able to change the billing account of a project that is billed to the specified org, to some other entity (corresponds to the restrictProjectTransfer org policy)",
    choices=["ADMIN", "MEMBER"],
)
...
parser_update_org.add_argument(
    ...
    choices=["true", "false"],
    help="If set to true, jobs launched in the projects billed to this org will collect detailed job metrics by default",
)
update_job_reuse_args = parser_update_org.add_mutually_exclusive_group(required=False)
update_job_reuse_args.add_argument(
    "--enable-job-reuse",
    action="store_true",
    help="Enable job reuse for projects where the org is the billTo",
)
update_job_reuse_args.add_argument(
    "--disable-job-reuse",
    action="store_true",
    help="Disable job reuse for projects where the org is the billTo",
)
parser_update_org.set_defaults(func=update_org)
register_parser(
    parser_update_org, subparsers_action=subparsers_update, categories="org"
)


parser_update_workflow = subparsers_update.add_parser(
    "workflow",
    help="Update the metadata for a workflow",
    description="Update the metadata for an existing workflow",
    parents=[stdout_args, env_args],
    prog="dx update workflow",
)
parser_update_workflow.add_argument(
    "workflow", help="Name or ID of a workflow"
).completer = DXPathCompleter(classes=["workflow"])
...
update_workflow_title_args.add_argument(
    "--no-title", help="Unset the workflow title", action="store_true"
)
...
update_workflow_output_folder_args = (
    parser_update_workflow.add_mutually_exclusive_group()
)
update_workflow_output_folder_args.add_argument(
    "--output-folder", help="Default output folder for the workflow"
)
update_workflow_output_folder_args.add_argument(
    "--no-output-folder",
    help="Unset the default output folder for the workflow",
    action="store_true",
)
parser_update_workflow.set_defaults(func=workflow_cli.update_workflow)
register_parser(
    parser_update_workflow, subparsers_action=subparsers_update, categories="workflow"
)

parser_update_stage = subparsers_update.add_parser(
    "stage",
    help="Update the metadata for a stage in a workflow",
    description="Update the metadata for a stage in a workflow",
    parents=[exec_input_args, stdout_args, env_args, instance_type_arg],
    prog="dx update stage",
)
parser_update_stage.add_argument(
    "workflow", help="Name or ID of a workflow"
).completer = DXPathCompleter(classes=["workflow"])
parser_update_stage.add_argument(
    "stage", help="Stage (index or ID) of the workflow to update"
)
parser_update_stage.add_argument(
    "--executable", help="Name or ID of an executable to replace in the stage"
).completer = MultiCompleter([DXAppCompleter(), DXPathCompleter(classes=["applet"])])
parser_update_stage.add_argument(
    "--alias",
    "--version",
...
update_stage_name_args.add_argument(
    "--no-name", help="Unset the stage name", action="store_true"
)
update_stage_folder_args = parser_update_stage.add_mutually_exclusive_group()
update_stage_folder_args.add_argument(
    "--output-folder",
    help="Path to the output folder for the stage (interpreted as an absolute path)",
)
update_stage_folder_args.add_argument(
...
register_parser(
    parser_update_stage, subparsers_action=subparsers_update, categories="workflow"
)

parser_update_member = subparsers_update.add_parser(
    "member",
    help="Update the membership of a user in an org",
    description="Update the membership of a user in an org",
    prog="dx update member",
    parents=[stdout_args, env_args],
)
...
parser_update_member.add_argument(
    "--level",
    choices=["ADMIN", "MEMBER"],
    help="The new org membership level of the specified user",
)
parser_update_member.add_argument(
    "--allow-billable-activities",
    choices=["true", "false"],
    help='The new "allowBillableActivities" membership permission of the specified user in the org; default false if demoting the specified user from ADMIN to MEMBER',
)
parser_update_member.add_argument(
    "--app-access",
    choices=["true", "false"],
    help='The new "appAccess" membership permission of the specified user in the org; default true if demoting the specified user from ADMIN to MEMBER',
)
parser_update_member.add_argument(
    "--project-access",
    choices=["ADMINISTER", "CONTRIBUTE", "UPLOAD", "VIEW", "NONE"],
    help="The new default implicit maximum permission the specified user will receive to projects explicitly shared with the org; default CONTRIBUTE if demoting the specified user from ADMIN to MEMBER",
)
parser_update_member.set_defaults(func=update_membership)
register_parser(
    parser_update_member, subparsers_action=subparsers_update, categories="org"
)

parser_update_project = subparsers_update.add_parser(
    "project",
    help="Updates a specified project with the specified options",
    description="",
    prog="dx update project",
    parents=[stdout_args, env_args],
)
...
parser_update_project.add_argument(
    "--containsPHI", choices=["true"], help="Flag to tell if project contains PHI"
)
parser_update_project.add_argument(
...
parser_update_project.add_argument(
    "--bill-to", help="Update the user or org ID of the billing account", type=str
)
allowed_executables_group = parser_update_project.add_mutually_exclusive_group()
allowed_executables_group.add_argument(
    "--allowed-executables",
    help="Executable ID(s) this project is allowed to run.  This operation overrides any existing list of executables.",
    type=str,
    nargs="+",
)
allowed_executables_group.add_argument(
    "--unset-allowed-executables",
    help="Removes any restriction to run executables as set by --allowed-executables",
    action="store_true",
)

parser_update_project.set_defaults(func=update_project)
register_parser(
    parser_update_project, subparsers_action=subparsers_update, categories="metadata"
)
...
parser_install = subparsers.add_parser(
    "install",
    help="Install an app",
    description='Install an app by name.  To see a list of apps you can install, hit <TAB> twice after "dx install" or run "'
    + BOLD("dx find apps")
    + '" to see a list of available apps.',
    prog="dx install",
    parents=[env_args],
)
install_app_action = parser_install.add_argument(
    "app", help="ID or name of app to install"
)
...
parser_uninstall = subparsers.add_parser(
    ...
    description="Uninstall an app by name.",
    prog="dx uninstall",
    parents=[env_args],
)
uninstall_app_action = parser_uninstall.add_argument(
    "app", help="ID or name of app to uninstall"
)
...
parser_run = subparsers.add_parser(
    "run",
    help="Run an applet, app, or workflow",
    add_help=False,
    description=(
        fill(
            'Run an applet, app, or workflow.  To see a list of executables you can run, hit <TAB> twice after "dx run" or run "'
            + BOLD("dx find apps")
            + '" or "'
            + BOLD("dx find globalworkflows")
            + '" to see a list of available apps and global workflows.'
        )
        + "\n\n"
        + fill(
            'If any inputs are required but not specified, an interactive mode for selecting inputs will be launched.  Inputs can be set in multiple ways.  Run "'
            + BOLD("dx run --input-help")
            + '" for more details.'
        )
        + "\n\n"
        + fill(
            'Run "'
            + BOLD("dx run --instance-type-help")
            + '" to see a list of specifications for computers available to run executables.'
        )
    ),
    prog="dx run",
    formatter_class=argparse.RawTextHelpFormatter,
...
run_executable_action = parser_run.add_argument(
    "executable",
    help=fill(
        "Name or ID of an applet, app, or workflow to run; must be provided if --clone is not set",
        width_adjustment=-24,
    ),
    nargs="?",
    default="",
)
run_executable_action.completer = MultiCompleter(
    [
        DXAppCompleter(),
...
parser_run.add_argument(
    ...
    "--depends-on",
    help=fill(
        'ID of job, analysis, or data object that must be in the "done" or '
...
parser_run.add_argument(
    ...
    dest="alias",
    help=fill(
        'Alias (tag) or version of the app to run (default: "default" if an app)',
        width_adjustment=-24,
    ),
)
parser_run.add_argument(
    ...
    dest="folder",
    help=fill(
        "The full project:folder path in which to output the results. By default, the current working directory will be used.",
        width_adjustment=-24,
    ),
)
parser_run.add_argument(
    ...
    dest="batch_folders",
    help=fill(
        "Output results to separate folders, one per batch, using batch ID as the name of the output folder. The batch output folder location will be relative to the path set in --destination",
        width_adjustment=-24,
    ),
    action="store_true",
)
parser_run.add_argument(
    ...
    metavar="PROJECT",
    help=fill(
        "Project name or ID in which to run the executable. This can also "
...
parser_run.add_argument(
    "--stage-output-folder",
    metavar=("STAGE_ID", "FOLDER"),
    help=fill(
        "A stage identifier (ID, name, or index), and a folder path to "
...
parser_run.add_argument(
    "--stage-relative-output-folder",
    metavar=("STAGE_ID", "FOLDER"),
    help=fill(
        "A stage identifier (ID, name, or index), and a relative folder "
...
parser_run.add_argument(
    "--name",
    help=fill(
        "Name for the job (default is the app or applet name)", width_adjustment=-24
    ),
)
parser_run.add_argument(
    "--delay-workspace-destruction",
    help=fill(
        "Whether to keep the job's temporary workspace around for debugging purposes for 3 days after it succeeds or fails",
        width_adjustment=-24,
    ),
    action="store_true",
)
...
parser_run.add_argument(
    ...
    action="store_true",
    help=fill(
        "Requests that the head job of an app or applet be run in an on-demand instance. "
...
parser_run.add_argument(
    "--wait", help="Wait until the job is done before returning", action="store_true"
)
parser_run.add_argument(
    "--watch",
    help="Watch the job after launching it. Defaults --priority to high.",
    action="store_true",
)
parser_run.add_argument(
    "--allow-ssh",
    ...
    metavar="ADDRESS",
    help=fill(
        "Configure the job to allow SSH access. Defaults --priority to high. If an argument is "
...
parser_run.add_argument(
    "--ssh",
    help=fill(
        "Configure the job to allow SSH access and connect to it after launching. "
...
parser_run.add_argument(
    ...
    help=fill(
        "SSH connect via proxy, argument supplied is used as the proxy address and port",
        width_adjustment=-24,
    ),
)
parser_run.add_argument(
    "--debug-on",
    action="append",
    choices=["AppError", "AppInternalError", "ExecutionError", "All"],
    help=fill(
        "Configure the job to hold for debugging when any of the listed errors occur",
        width_adjustment=-24,
    ),
)

ignore_reuse = parser_run.add_mutually_exclusive_group()
ignore_reuse.add_argument(
    "--ignore-reuse",
    help=fill("Disable job reuse for execution", width_adjustment=-24),
    action="store_true",
)
ignore_reuse.add_argument(
    "--ignore-reuse-stage",
    metavar="STAGE_ID",
    dest="ignore_reuse_stages",
    help=fill(
        "A stage (using its ID, name, or index) for which job reuse should be disabled, "
...
parser_run.add_argument(
    ...
    dest="rerun_stages",
    help=fill(
        'A stage (using its ID, name, or index) to rerun, or "*" to '
...
parser_run.add_argument(
    ...
    metavar="FILE",
    help=fill(
        "A file in tab separated value (tsv) format, with a subset "
...
parser_run.add_argument(
    ...
    help=fill(
        "Specify spark cluster instance count(s). It can be an int or a mapping of the format {ic}".format(
            ic=ic_format
        ),
        width_adjustment=-24,
    ),
...
parser_run.add_argument(
    "--input-help",
    help=fill(
        "Print help and examples for how to specify inputs", width_adjustment=-24
    ),
    action=runInputHelp,
    nargs=0,
)
parser_run.add_argument(
    "--detach",
    help=fill(
        "When invoked from a job, detaches the new job from the creator job so the "
...
parser_run.add_argument(
    "--cost-limit",
    help=fill(
        "Maximum cost of the job before termination. In case of workflows it is cost of the "
...
parser_run.add_argument(
    ...
    default=None,
    help=fill(
        "Set the rank of the root execution, integer between -1024 and 1023. Requires executionRankEnabled license feature for the billTo. Default is 0.",
        width_adjustment=-24,
    ),
)
parser_run.add_argument(
    "--max-tree-spot-wait-time",
    help=fill(
...
parser_run.add_argument(
    ...
    default=None,
    help=fill(
        "Collect CPU, memory, network and disk metrics every 60 seconds",
        width_adjustment=-24,
    ),
)

preserve_outputs = parser_run.add_mutually_exclusive_group()
preserve_outputs.add_argument(
    "--preserve-job-outputs",
    action="store_true",
    help=fill(
        'Copy cloneable outputs of every non-reused job entering "done" state in this '
...
parser_watch = subparsers.add_parser(
    "watch",
    help="Watch logs of a job and its subjobs",
    prog="dx watch",
...
parser_watch.add_argument(
    "-n",
    "--num-recent-messages",
    help="Number of recent messages to get",
    type=int,
    default=1024 * 256,
)
parser_watch.add_argument(
    "--tree", help="Include the entire job tree", action="store_true"
)
parser_watch.add_argument(
    "-l",
    "--levels",
...
parser_watch.add_argument(
    "--get-stdout", help="Extract stdout only from this job", action="store_true"
)
parser_watch.add_argument(
    "--get-stderr", help="Extract stderr only from this job", action="store_true"
)
parser_watch.add_argument(
    "--get-streams",
    help="Extract only stdout and stderr from this job",
    action="store_true",
)
parser_watch.add_argument(
    "--no-timestamps",
    help="Omit timestamps from messages",
    action="store_false",
    dest="timestamps",
)
parser_watch.add_argument(
    "--job-ids", help="Print job ID in each message", action="store_true"
)
parser_watch.add_argument(
    "--no-job-info",
    help="Omit job info and status updates",
    action="store_false",
    dest="job_info",
)
parser_watch.add_argument(
    "-q", "--quiet", help="Do not print extra info messages", action="store_true"
)
parser_watch.add_argument(
    "-f", "--format", help="Message format. Available fields: job, level, msg, date"
)
parser_watch.add_argument(
    "--no-wait",
    "--no-follow",
...
parser_watch.add_argument(
    "--metrics",
    help=fill(
        "Select display mode for detailed job metrics if they were collected and are available based on retention policy; see --metrics-help for details",
        width_adjustment=-24,
    ),
    choices=["interspersed", "none", "csv"],
...
parser_watch.add_argument(
    "--metrics-help",
    action=MetricsHelpAction,
    nargs=0,
    help="Print help for displaying detailed job metrics",
)
...
parser_ssh_config = subparsers.add_parser(
    "ssh_config",
    help="Configure SSH keys for your DNAnexus account",
    description="Configure SSH access credentials for your DNAnexus account",
    prog="dx ssh_config",
    parents=[env_args],
)
parser_ssh_config.add_argument(
    "ssh_keygen_args",
    help="Command-line arguments to pass to ssh-keygen",
    nargs=argparse.REMAINDER,
)
parser_ssh_config.add_argument(
    "--revoke",
    help="Revoke SSH public key associated with your DNAnexus account; you will no longer be able to SSH into any jobs.",
    action="store_true",
)
...
parser_ssh = subparsers.add_parser(
    "ssh",
    help="Connect to a running job via SSH",
...
parser_ssh.add_argument(
    "ssh_args",
    help="Command-line arguments to pass to the SSH client",
    nargs=argparse.REMAINDER,
)
parser_ssh.add_argument(
    "--ssh-proxy",
...
parser_ssh_firewall.add_argument(
    "--no-firewall-update",
    help="Do not update the allowSSH allowed IP ranges before connecting with ssh",
    action="store_true",
    default=False,
)
parser_ssh_firewall.add_argument(
    ...
    metavar="ADDRESS",
    help=fill(
        'Configure the job to allow SSH access from an IP range, e.g. "--allow-ssh 1.2.3.4". '
        + "If no argument is supplied then the client IP visible to the DNAnexus API server will be used by default",
        width_adjustment=-24,
    ),
)
...
parser_terminate = subparsers.add_parser(
    "terminate",
    help="Terminate jobs or analyses",
    description="Terminate one or more jobs or analyses",
    prog="dx terminate",
    parents=[env_args],
)
parser_terminate.add_argument(
    "jobid", help="ID of a job or analysis to terminate", nargs="+"
)
parser_terminate.set_defaults(func=terminate)
parser_map["terminate"] = parser_terminate
parser_categories["all"]["cmds"].append(
    (subparsers._choices_actions[-1].dest, subparsers._choices_actions[-1].help)
)
parser_categories["exec"]["cmds"].append(
    (subparsers._choices_actions[-1].dest, subparsers._choices_actions[-1].help)
)
...
# rmproject
#####################################
parser_rmproject = subparsers.add_parser(
    "rmproject",
...
projects_action = parser_rmproject.add_argument(
    "projects", help="Projects to remove", metavar="project", nargs="+"
)
projects_action.completer = DXPathCompleter(
    expected="project", include_current_proj=True
)
parser_rmproject.add_argument(
    "-y",
    "--yes",
    dest="confirm",
    help="Do not ask for confirmation",
...
parser_rmproject.add_argument(
    "-q",
    "--quiet",
    help="Do not print purely informational messages",
    action="store_true",
)
...
parser_new = subparsers.add_parser(
    "new",
    help="Create a new project or data object",
...
parser_new_user_user_opts.add_argument(
    "--token-duration",
    help='Time duration for which the newly generated auth token for the new user will be valid (default 30 days; max 30 days). An integer will be interpreted as seconds; you can append a suffix (s, m, h, d) to indicate different units (e.g. "--token-duration 10m" to indicate 10 minutes).',
)
parser_new_user_user_opts.add_argument("--occupation", help="Occupation")
parser_new_user_user_opts.add_argument(
    "--on-behalf-of", help="On behalf of which org is the account provisioned"
)
parser_new_user_org_opts = parser_new_user.add_argument_group(
    "Org options",
    "Optionally invite the new user to an org with the specified parameters",
)
...
parser_new_user_org_opts.add_argument(
    ...
    choices=["ADMIN", "MEMBER"],
    default="MEMBER",
    action=DXNewUserOrgArgsAction,
    help="Org membership level that will be granted to the new user; default MEMBER",
)
parser_new_user_org_opts.add_argument(
    "--set-bill-to",
    default=False,
    action=DXNewUserOrgArgsAction,
    help='Set the default "billTo" field of the new user to the org; implies --allow-billable-activities',
)
parser_new_user_org_opts.add_argument(
    "--allow-billable-activities",
    default=False,
    action=DXNewUserOrgArgsAction,
    help='Grant the new user "allowBillableActivities" in the org',
)
parser_new_user_org_opts.add_argument(
    "--no-app-access",
    default=True,
...
parser_new_user_org_opts.add_argument(
    ...
    default="CONTRIBUTE",
    action=DXNewUserOrgArgsAction,
    help='The "projectAccess" to grant the new user in the org; default CONTRIBUTE',
)
parser_new_user_org_opts.add_argument(
    "--no-email",
    default=False,
...
register_parser(parser_new_user, subparsers_action=subparsers_new, categories="other")

parser_new_org = subparsers_new.add_parser(
    "org",
    help="Create new non-billable org",
...
parser_new_org.add_argument(
    "--handle",
    required=True,
    help='Unique handle for the org. The specified handle will be converted to lowercase and appended to "org-" to form the org ID',
)
parser_new_org.add_argument(
    "--member-list-visibility",
    default="ADMIN",
    help="Org membership level required to be able to list the members of the org, or to view the membership level or permissions of any other member of the org; default ADMIN",
    choices=["ADMIN", "MEMBER", "PUBLIC"],
)
parser_new_org.add_argument(
    "--project-transfer-ability",
    default="ADMIN",
    help="Org membership level required to be able to change the billing account of an org-billed project to any other entity; default ADMIN",
    choices=["ADMIN", "MEMBER"],
)
parser_new_org.set_defaults(func=new_org)
register_parser(parser_new_org, subparsers_action=subparsers_new, categories="org")

parser_new_project = subparsers_new.add_parser(
    "project",
...
parser_new_project.add_argument(
    "-s",
    "--select",
    help="Select the new project as current after creating",
    action="store_true",
)
parser_new_project.add_argument(
    "--bill-to",
    help="ID of the user or org to which the project will be billed. The default value is the billTo of the requesting user.",
)
parser_new_project.add_argument(
    "--phi", help="Add PHI protection to project", default=False, action="store_true"
)
parser_new_project.add_argument(
    "--database-ui-view-only",
    help="Viewers on the project cannot access database data directly",
    default=False,
    action="store_true",
)
...
parser_new_record = subparsers_new.add_parser(
    "record",
...
init_action = parser_new_record.add_argument(
    "--init", help="Path to record from which to initialize all metadata"
)
parser_new_record.add_argument(
    "--close",
    help="Close the record immediately after creating it",
    action="store_true",
)
...
parser_new_workflow = subparsers_new.add_parser(
    "workflow",
...
parser_new_workflow.add_argument(
    "--output-folder", help="Default output folder for the workflow"
)
init_action = parser_new_workflow.add_argument(
    "--init",
    help=fill(
        "Path to workflow or an analysis ID from which to initialize all metadata",
        width_adjustment=-24,
    ),
)
...
register_parser(
    parser_new_workflow, subparsers_action=subparsers_new, categories="workflow"
)

#####################################
# get_details
#####################################
parser_get_details = subparsers.add_parser(
    "get_details",
    help="Get details of a data object",
    description="Get the JSON details of a data object.",
    prog="dx get_details",
    parents=[env_args],
)
parser_get_details.add_argument(
    "path", help="ID or path to data object to get details for"
).completer = DXPathCompleter()
...
# set_details
#####################################
parser_set_details = subparsers.add_parser(
    "set_details",
    help="Set details on a data object",
    description="Set the JSON details of a data object.",
    prog="dx set_details",
    parents=[env_args, all_arg],
)
parser_set_details.add_argument(
    "path", help="ID or path to data object to modify"
).completer = DXPathCompleter()
parser_set_details.add_argument("details", help="JSON to store as details", nargs="?")
parser_set_details.add_argument(
    "-f",
    "--details-file",
    help="Path to local file containing JSON to store as details",
)
...
# set_visibility
#####################################
parser_set_visibility = subparsers.add_parser(
    "set_visibility",
    help="Set visibility on a data object",
    description="Set visibility on a data object.",
    prog="dx set_visibility",
    parents=[env_args, all_arg],
)
parser_set_visibility.add_argument(
    "path", help="ID or path to data object to modify"
).completer = DXPathCompleter()
parser_set_visibility.add_argument(
    "visibility",
...
# add_types
#####################################
parser_add_types = subparsers.add_parser(
    "add_types",
    help="Add types to a data object",
...
# remove_types
#####################################
parser_remove_types = subparsers.add_parser(
    "remove_types",
    help="Remove types from a data object",
...
parser_remove_types.add_argument(
    "path", help="ID or path to data object to modify"
).completer = DXPathCompleter()
parser_remove_types.add_argument(
    "types", nargs="+", metavar="type", help="Types to remove"
)
...
parser_tag = subparsers.add_parser(
    "tag",
    help="Tag a project, data object, or execution",
    prog="dx tag",
...
parser_tag.add_argument(
    "path", help="ID or path to project, data object, or execution to modify"
).completer = DXPathCompleter()
...
parser_untag = subparsers.add_parser(
    "untag",
    help="Untag a project, data object, or execution",
    prog="dx untag",
...
path_action = parser_rename.add_argument(
    "path", help="Path to project or data object to rename"
)
...
# set_properties
#####################################
parser_set_properties = subparsers.add_parser(
    "set_properties",
    help="Set properties of a project, data object, or execution",
    description="Set properties of a project, data object, or execution.  Note that a project context must be either set or specified for data object IDs or paths.",
    prog="dx set_properties",
    parents=[env_args, all_arg],
)
parser_set_properties.add_argument(
    "path", help="ID or path to project, data object, or execution to modify"
).completer = DXPathCompleter()
parser_set_properties.add_argument(
    "properties",
    nargs="+",
    metavar="propertyname=value",
...
# unset_properties
#####################################
parser_unset_properties = subparsers.add_parser(
    "unset_properties",
    help="Unset properties of a project, data object, or execution",
...
path_action = parser_unset_properties.add_argument(
    "path", help="ID or path to project, data object, or execution to modify"
)
path_action.completer = DXPathCompleter()
parser_unset_properties.add_argument(
    "properties", nargs="+", metavar="propertyname", help="Property names to unset"
)
...
parser_close = subparsers.add_parser(
    "close",
    help="Close data object(s)",
    description="Close a remote data object or set of objects.",
    prog="dx close",
    parents=[env_args, all_arg],
)
parser_close.add_argument(
    "path", help="Path to a data object to close", nargs="+"
).completer = DXPathCompleter()
parser_close.add_argument(
    "--wait", help="Wait for the object(s) to close", action="store_true"
)
...
parser_wait = subparsers.add_parser(
    "wait",
    help="Wait for data object(s) to close or job(s) to finish",
...
path_action = parser_wait.add_argument(
    "path",
    help="Path to a data object, job ID, or file with IDs to wait for",
    nargs="+",
)
path_action.completer = DXPathCompleter()
parser_wait.add_argument(
    "--from-file",
    help="Read the list of objects to wait for from the file provided in path",
    action="store_true",
)
...
parser_get = subparsers.add_parser(
    "get",
    help="Download records, apps, applets, workflows, files, and databases.",
...
parser_get.add_argument(
    "path", help="Data object ID or name to access"
).completer = DXPathCompleter(
    classes=["file", "record", "applet", "app", "workflow", "database"]
)
parser_get.add_argument(
    "-o",
    "--output",
    help="local file path where the data is to be saved (\"-\" indicates stdout output for objects of class file and record). If not supplied, the object's name on the platform will be used, along with any applicable extensions. For app(let) and workflow objects, if OUTPUT does not exist, the object's source directory will be created there; if OUTPUT is an existing directory, a new directory with the object's name will be created inside it.",
)
parser_get.add_argument(
    "--filename",
    default="/",
    help="When downloading from a database, name of the file or folder to be downloaded. If omitted, all files in the database will be downloaded, so use caution and include the --allow-all-files argument.",
)
parser_get.add_argument(
    "--allow-all-files",
    default=False,
    help="When downloading from a database, this allows all files in a database to be downloaded when --filename argument is omitted.",
    action="store_true",
    dest="allow_all_files",
)
parser_get.add_argument(
    "--recurse",
    default=False,
    help="When downloading from a database, look for files recursively down the directory structure. Otherwise, by default, only look on one level.",
    action="store_true",
)
parser_get.add_argument(
    "--no-ext",
    help="If -o is not provided, do not add an extension to the filename",
    action="store_true",
)
parser_get.add_argument(
    "--omit-resources",
    help="When downloading an app(let), omit fetching the resources associated with the app(let).",
    action="store_true",
)
parser_get.add_argument(
    "-f",
    "--overwrite",
    help="Overwrite the local file if necessary",
    action="store_true",
)
...
parser_find = subparsers.add_parser(
    "find",
    help="Search functionality over various DNAnexus entities",
...
parser_find_apps = subparsers_find.add_parser(
    ...
    description=fill(
        "Finds apps subject to the given search parameters. Use --category to restrict by a category; "
...
parser_find_apps.add_argument(
    "--category", help="Category of the app"
).completer = ListCompleter(APP_CATEGORIES)
parser_find_apps.add_argument(
    "--category-help",
...
parser_find_apps.add_argument(
    "-a", "--all", help="Return all versions of each app", action="store_true"
)
parser_find_apps.add_argument(
    "--unpublished",
    help="Return only unpublished apps (if omitted, returns only published apps)",
    action="store_true",
)
parser_find_apps.add_argument(
    "--installed", help="Return only installed apps", action="store_true"
)
parser_find_apps.add_argument(
    "--billed-to", help="User or organization responsible for the app"
)
...
parser_find_apps.add_argument(
    "--created-after",
    help="""Date (e.g. --created-after="2021-12-01" or --created-after="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-after=1642196636000) after which the app created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-after=-2d for apps created in the last 2 days)""",
)
parser_find_apps.add_argument(
    "--created-before",
    help="""Date (e.g. --created-before="2021-12-01" or --created-before="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-before=1642196636000) before which the app was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-before=-2d for apps created earlier than 2 days ago)""",
)
parser_find_apps.add_argument(
    "--mod-after",
    help="""Date (e.g. --mod-after="2021-12-01" or --mod-after="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --mod-after=1642196636000) after which the app modified. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --mod-after=-2d for apps modified in the last 2 days)""",
)
parser_find_apps.add_argument(
    "--mod-before",
    help="""Date (e.g. --mod-before="2021-12-01" or --mod-before="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --mod-before=1642196636000) after which the app modified. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --mod-before=-2d for apps modified earlier than 2 days ago)""",
)
...
parser_find_globalworkflows = subparsers_find.add_parser(
    ...
    description=fill(
        "Finds global workflows subject to the given search parameters. Use --category to restrict by a category; "
        "common categories are available as tab completions and can be listed with --category-help."
    ),
    parents=[stdout_args, json_arg, delim_arg, env_args],
...
parser_find_globalworkflows.add_argument(
    "--category", help="Category of the workflow"
).completer = ListCompleter(APP_CATEGORIES)
parser_find_globalworkflows.add_argument(
    "--category-help",
...
parser_find_globalworkflows.add_argument(
    "-a", "--all", help="Return all versions of each workflow", action="store_true"
)
parser_find_globalworkflows.add_argument(
    "--unpublished",
    help="Return only unpublished workflows (if omitted, returns only published workflows)",
    action="store_true",
)
parser_find_globalworkflows.add_argument(
    "--billed-to", help="User or organization responsible for the workflow"
)
parser_find_globalworkflows.add_argument(
    "--creator", help="Creator of the workflow version"
)
parser_find_globalworkflows.add_argument(
    "--developer", help="Developer of the workflow"
)
parser_find_globalworkflows.add_argument(
    "--created-after",
    help="""Date (e.g. --created-after="2021-12-01" or --created-after="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-after=1642196636000) after which the workflow was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-after=-2d for workflows created in the last 2 days).""",
)
parser_find_globalworkflows.add_argument(
    "--created-before",
    help="""Date (e.g. --created-before="2021-12-01" or --created-before="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-before=1642196636000) before which the workflow was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-before=-2d for workflows created earlier than 2 days ago)""",
)
parser_find_globalworkflows.add_argument(
    "--mod-after",
    help="""Date (e.g. --mod-after="2021-12-01" or --mod-after="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --mod-after=1642196636000) after which the workflow was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --mod-after=-2d for workflows modified in the last 2 days)""",
)
parser_find_globalworkflows.add_argument(
    "--mod-before",
    help="""Date (e.g. --mod-before="2021-12-01" or --mod-before="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --mod-before=1642196636000) before which the workflow was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --mod-before=-2d for workflows modified earlier than 2 days ago)""",
)
parser_find_globalworkflows.set_defaults(func=find_global_workflows)
register_parser(
    parser_find_globalworkflows, subparsers_action=subparsers_find, categories="exec"
)
...
parser_find_jobs = subparsers_find.add_parser(
    ...
    description=fill(
        "Finds jobs subject to the given search parameters. By default, output is formatted to show the "
...
parser_find_analyses = subparsers_find.add_parser(
    ...
    description=fill(
        "Finds analyses subject to the given search parameters. By default, output is formatted to show "
...
register_parser(
    parser_find_analyses, subparsers_action=subparsers_find, categories="exec"
)
...
parser_find_executions = subparsers_find.add_parser(
    ...
    description=fill(
        "Finds executions (jobs and analyses) subject to the given search parameters. By default, output "
...
register_parser(
    parser_find_executions, subparsers_action=subparsers_find, categories="exec"
)
...
parser_find_data = subparsers_find.add_parser(
    ...
    description=fill(
        "Finds data objects subject to the given search parameters. By default, restricts the search to "
...
parser_find_data.add_argument(
    "--class",
    dest="classname",
    choices=["record", "file", "applet", "workflow", "database"],
    help="Data object class",
    metavar="{record,file,applet,workflow,database}",
)
parser_find_data.add_argument(
    "--state", choices=["open", "closing", "closed", "any"], help="State of the object"
)
parser_find_data.add_argument(
    "--visibility",
    choices=["hidden", "visible", "either"],
    default="visible",
    help="Whether the object is hidden or not",
)
...
parser_find_data.add_argument(
    "--all-projects",
    "--allprojects",
    help="Extend search to all projects (excluding public projects)",
    action="store_true",
)
parser_find_data.add_argument("--project", help=argparse.SUPPRESS)
parser_find_data.add_argument(
    "--folder", help=argparse.SUPPRESS
).completer = DXPathCompleter(expected="folder")
parser_find_data.add_argument(
    "--path",
    help="Project and/or folder in which to restrict the results",
...
parser_find_data.add_argument(
    "--created-after",
    help="""Date (e.g. --created-after="2021-12-01" or --created-after="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-after=1642196636000) after which the object was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-after=-2d for objects created in the last 2 days).""",
)
parser_find_data.add_argument(
    "--created-before",
    help="""Date (e.g. --created-before="2021-12-01" or --created-before="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-before=1642196636000) before which the object was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-before=-2d for objects created earlier than 2 days ago)""",
)
parser_find_data.add_argument(
    "--mod-after",
    help="""Date (e.g. --mod-after="2021-12-01" or --mod-after="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --mod-after=1642196636000) after which the object was modified. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --mod-after=-2d for objects modified in the last 2 days)""",
)
parser_find_data.add_argument(
    "--mod-before",
    help="""Date (e.g. --mod-before="2021-12-01" or --mod-before="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --mod-before=1642196636000) before which the object was modified. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --mod-before=-2d for objects modified earlier than 2 days ago)""",
)
parser_find_data.add_argument(
    "--region", help="Restrict the search to the provided region"
)

parser_find_data.set_defaults(func=find_data)
register_parser(
    parser_find_data, subparsers_action=subparsers_find, categories=("data", "metadata")
)
...
parser_find_projects = subparsers_find.add_parser(
    ...
    description=fill(
        "Finds projects subject to the given search parameters. Use the --public flag to list all public "
        "projects."
    ),
    parents=[
        stdout_args,
...
parser_find_projects.add_argument(
    "--level",
    choices=["VIEW", "UPLOAD", "CONTRIBUTE", "ADMINISTER"],
    help="Minimum level of permissions expected",
)
...
register_parser(
    parser_find_projects, subparsers_action=subparsers_find, categories="data"
)
...
parser_find_org_members.add_argument(
    "--level",
    choices=["ADMIN", "MEMBER"],
    help="Restrict the result set to contain only members at the specified membership level.",
)
parser_find_org_members.set_defaults(func=org_find_members)
register_parser(
    parser_find_org_members, subparsers_action=subparsers_find_org, categories="org"
)
...
parser_find_org_projects = subparsers_find_org.add_parser(
    ...
    description=fill(
        "Finds projects billed to the specified org subject to the given search parameters. You must "
...
parser_find_org_projects.add_argument(
    "--ids",
    nargs="+",
    help='Possible project IDs. May be specified like "--ids project-1 project-2"',
)
find_org_projects_public = parser_find_org_projects.add_mutually_exclusive_group()
find_org_projects_public.add_argument(
    "--public-only",
    dest="public",
    help="Include only public projects",
    action="store_true",
    default=None,
)
find_org_projects_public.add_argument(
    "--private-only",
    dest="public",
    help="Include only private projects",
    action="store_false",
    default=None,
)
parser_find_org_projects.add_argument(
...
register_parser(
    ...
    subparsers_action=subparsers_find_org,
    categories=("data", "org"),
)
...
parser_find_org_apps = subparsers_find_org.add_parser(
    ...
    description=fill(
        "Finds apps billed to the specified org subject to the given search parameters. You must "
...
parser_find_org_apps.add_argument(
    "--category", help="Category of the app"
).completer = ListCompleter(APP_CATEGORIES)
parser_find_org_apps.add_argument(
    "--category-help",
...
parser_find_org_apps.add_argument(
    "--unpublished",
    help="Return only unpublished apps (if omitted, returns all apps)",
    action="store_true",
)
parser_find_org_apps.add_argument(
    "--installed", help="Return only installed apps", action="store_true"
...
parser_find_org_apps.add_argument(
    "--created-after",
    help="""Date (e.g. --created-after="2021-12-01" or --created-after="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-after=1642196636000) after which the app was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-after=-2d for apps created in the last 2 days).""",
)
parser_find_org_apps.add_argument(
    "--created-before",
    help="""Date (e.g. --created-before="2021-12-01" or --created-before="2021-12-01 19:01:33") or integer Unix epoch timestamp in milliseconds (e.g. --created-before=1642196636000) before which the app was created. You can also specify negative numbers to indicate a time period in the past suffixed by s, m, h, d, w, M or y to indicate seconds, minutes, hours, days, weeks, months or years (e.g. --created-before=-2d for apps created earlier than 2 days ago)""",
...
parser_find_org_apps.add_argument(
    "--mod-after",
    help='''Date (e.g. 2012-01-01) or integer timestamp after which the app was last modified (negative number means seconds in the past, or use suffix s, m, h, d, w, M, y) 
                                                         Negative input example "--mod-after=-2d"''',
)
parser_find_org_apps.add_argument(
    "--mod-before",
    help='''Date (e.g. 2012-01-01) or integer timestamp before which the app was last modified (negative number means seconds in the past, or use suffix s, m, h, d, w, M, y) 
...
parser_find_orgs.add_argument(
    "--level",
    choices=["ADMIN", "MEMBER"],
    required=True,
    help="Restrict the result set to contain only orgs in which the requesting user has at least the specified membership level",
)
parser_find_orgs_with_billable_activities = (
    parser_find_orgs.add_mutually_exclusive_group()
)
parser_find_orgs_with_billable_activities.add_argument(
    "--with-billable-activities",
    action="store_true",
    help="Restrict the result set to contain only orgs in which the requesting user can perform billable activities; mutually exclusive with --without-billable-activities",
)
parser_find_orgs_with_billable_activities.add_argument(
    "--without-billable-activities",
    dest="with_billable_activities",
    action="store_false",
    help="Restrict the result set to contain only orgs in which the requesting user **cannot** perform billable activities; mutually exclusive with --with-billable-activities",
)
...
parser_notebook = subparsers.add_parser(
    "notebook",
    help="Launch a web notebook inside DNAnexus.",
    description="Launch a web notebook inside DNAnexus.",
    formatter_class=argparse.RawTextHelpFormatter,
    prog="dx notebook",
)
parser_notebook.add_argument(
    "notebook_type",
    help="Type of notebook to launch",
    choices=["jupyter_lab", "jupyter_notebook", "jupyter"],
)
parser_notebook.add_argument(
    "--notebook_files",
    help="Files to include on notebook instance",
    default=[],
    nargs="*",
).completer = DXPathCompleter(classes=["file"])
parser_notebook.add_argument(
    "--spark", help="Install spark infrastructure.", action="store_true", default=False
)
parser_notebook.add_argument(
    "--port", help="local port to use to access the notebook.", default="2001"
)
parser_notebook.add_argument(
    "--snapshot", help="A snapshot file to reform on the server."
).completer = DXPathCompleter(classes=["file"])
parser_notebook.add_argument(
    "--timeout", help="How long to keep the notebook open (smhwMy).", default="1h"
)
parser_notebook.add_argument(
    "-q",
    "--quiet",
...
parser_notebook.add_argument(
    "--version", help="What version of the notebook app to launch.", default=None
)
parser_notebook.add_argument(
    "--instance-type",
    help="Instance type to run the notebook on.",
    default="mem1_ssd1_x4",
)
parser_notebook.add_argument(
    "--only_check_config",
    help="Only check SSH config do not launch app",
    action="store_true",
)
notebook_with_ssh_config_check = functools.partial(
    run_notebook, ssh_config_check=verify_ssh_config
)
...
parser_loupe_viewer = subparsers.add_parser(
    "loupe-viewer",
    help="Launch the Loupe viewer for 10x data on DNAnexus.",
    description="Launch the Loupe viewer for 10x data on DNAnexus.",
    formatter_class=argparse.RawTextHelpFormatter,
    prog="dx loupe-viewer",
)
parser_loupe_viewer.add_argument(
    "loupe_files",
    help="Files to include in loupe viewer",
    default=[],
    nargs=argparse.REMAINDER,
).completer = DXPathCompleter(classes=["file"])
parser_loupe_viewer.add_argument(
    "--port", help="local port to use to access the Loupe viewer.", default="2001"
)
parser_loupe_viewer.add_argument(
    "--timeout", help="How long to keep the Loupe viewer open (smhwMy).", default="1h"
)
parser_loupe_viewer.add_argument(
    "-q",
    "--quiet",
...
parser_loupe_viewer.add_argument(
    "--instance-type",
    help="Instance type to run the Loupe viewer on.",
    default="mem1_ssd1_x4",
)
...
parser_api = subparsers.add_parser(
    "api",
    help="Call an API method",
    formatter_class=argparse.RawTextHelpFormatter,
    description=fill(
        "Call an API method directly.  The JSON response from the API server will be returned if successful.  No name resolution is performed; DNAnexus IDs must always be provided.  The API specification can be found at"
    )
    + """
...
parser_api.add_argument(
    "resource",
    help=fill(
        'One of "system", a class name (e.g. "record"), or an entity ID such as "record-xxxx".  Use "app-name/1.0.0" to refer to version "1.0.0" of the app named "name".',
        width_adjustment=-17,
    ),
)
parser_api.add_argument(
    "method",
    help=fill(
        "Method name for the resource as documented by the API specification",
        width_adjustment=-17,
    ),
)
parser_api.add_argument(
    ...
    nargs="?",
    default="{}",
    help='JSON input for the method (if not given, "{}" is used)',
)
parser_api.add_argument(
    "--input",
    help=fill('Load JSON input from FILENAME ("-" to use stdin)', width_adjustment=-17),
)
...
parser_upgrade = subparsers.add_parser(
    "upgrade",
    help="Upgrade dx-toolkit (the DNAnexus SDK and this program)",
...
parser_generate_batch_inputs = subparsers.add_parser(
    "generate_batch_inputs",
    help="Generate a batch plan (one or more TSV files) for batch execution",
...
parser_generate_batch_inputs.add_argument(
    "-i",
    "--input",
    help=fill(
        'An input to be batch-processed "-i<input name>=<input pattern>" where <input_pattern> is a regular expression with a group corresponding to the desired region to match (e.g. "-iinputa=SRR(.*)_1.gz" "-iinputb=SRR(.*)_2.gz")',
        width_adjustment=-24,
    ),
    action="append",
)
parser_generate_batch_inputs.add_argument(
    "--path",
    help="Project and/or folder to which the search for input files will be restricted",
    metavar="PROJECT:FOLDER",
    default="",
).completer = DXPathCompleter(expected="folder")
parser_generate_batch_inputs.add_argument(
    "-o", "--output_prefix", help="Prefix for output file", default="dx_batch"
)
...
parser_publish = subparsers.add_parser(
    "publish",
    help="Publish an app or a global workflow",
    description="Release a version of the executable (app or global workflow) to authorized users.",
    prog="dx publish",
)
parser_publish.add_argument(
    "executable",
    help="ID or name and version of an app/global workflow, e.g. myqc/1.0.0",
).completer = DXPathCompleter(classes=["app", "globalworkflow"])
parser_publish.add_argument(
    "--no-default",
...
""".format(
        BOLD("the specified set files"),
        BOLD("a single specified folder"),
        BOLD("ONE project"),
    )
    + """
...
parser_archive.add_argument(
    ...
    + "\n"
    + fill(
        "See https://documentation.dnanexus.com/developer/api/data-containers/projects#api-method-project-xxxx-archive for details.",
        width_adjustment=-24,
    ),
    default=False,
    action="store_true",
...
parser_archive.add_argument(
    ...
    dest="recurse",
    help=fill(
        "When `path` refers to a single folder, this flag causes only files in the specified folder and not its subfolders to be archived. This flag has no impact when `path` input refers to a collection of files.",
        width_adjustment=-24,
    ),
    action="store_false",
)
...
parser_archive.add_argument(
    "path",
    help=fill(
        "May refer to a single folder or specify up to 1000 files inside a project.",
        width_adjustment=-24,
    ),
    default=[],
    nargs="+",
...
parser_archive_output = parser_archive.add_argument_group(
    title="Output",
    description='If -q option is not specified, prints "Tagged <count> file(s) for archival"',
)
...
parser_unarchive.add_argument(
    ...
    dest="recurse",
    help=fill(
        "When `path` refers to a single folder, this flag causes only files in the specified folder and not its subfolders to be unarchived. This flag has no impact when `path` input refers to a collection of files.",
        width_adjustment=-24,
    ),
    action="store_false",
)
...
parser_unarchive.add_argument_group(
    title="Output",
    description='If -q option is not specified, prints "Tagged <> file(s) for unarchival, totalling <> GB, costing <> "',
)
...
parser_extract_dataset.add_argument(
    ...
    action="store_true",
    default=False,
    help="If provided, the three dictionary files, <record_name>.data_dictionary.csv, <record_name>.entity_dictionary.csv, and <record_name>.codings.csv will be generated. Files will be comma delimited and written to the local working directory, unless otherwise specified using --delimiter and --output arguments. If any of the three dictionary files does not contain data (i.e. the dictionary is empty), then that particular file will not be created.",
)
parser_extract_dataset.add_argument(
    "--fields",
    nargs="+",
    help='A comma-separated string where each value is the phenotypic entity name and field name, separated by a dot.  For example: "<entity_name>.<field_name>,<entity_name>.<field_name>". If multiple entities are provided, field values will be automatically inner joined. If only the --fields argument is provided, data will be retrieved and returned. If both --fields and --sql arguments are provided, a SQL statement to retrieve the specified field data will be automatically generated and returned.',
)
parser_extract_dataset.add_argument(
    "--sql",
    action="store_true",
    default=False,
    help="If provided, a SQL statement (string) will be returned to query the set of entity.fields, instead of returning stored values from the set of entity.fields",
)
parser_extract_dataset.add_argument(
    "--delim",
    "--delimiter",
...
parser_extract_dataset.add_argument(
    "-o",
    "--output",
    help='Local filename or directory to be used ("-" indicates stdout output). If not supplied, output will create a file with a default name in the current folder',
)
parser_extract_dataset.add_argument(
    "--list-fields",
    action="store_true",
    default=False,
    help='List the names and titles of all fields available in the dataset specified. When not specified together with "–-entities", it will return all the fields from the main entity. Output will be a two column table, field names and field titles, separated by a tab, where field names will be of the format, "<entity name>.<field name>" and field titles will be of the format, "<field title>".',
)
parser_extract_dataset.add_argument(
    "--list-entities",
    action="store_true",
    default=False,
    help="List the names and titles of all the entities available in the dataset specified. Output will be a two column table, entity names and entity titles, separated by a tab.",
)
parser_extract_dataset.add_argument(
    "--entities",
    help='Similar output to "--list-fields", however using "--entities" will allow for specific entities to be specified. When multiple entities are specified, use comma as the delimiter. For example: "--list-fields --entities entityA,entityB,entityC"',
)
...
parser_extract_assay_germline.add_argument(
    "-o", "--output", type=str, default=None, help="Path to store the output file."
)
...
parser_e_a_s_mutex_group = parser_extract_assay_somatic.add_mutually_exclusive_group(
    required=True
)
...
category_list = "\n  ".join(
    [
        category + parser_categories[category]["desc"]
        for category in parser_categories_sorted
    ]
)
parser_help = subparsers.add_parser(
    ...
    description=fill(
        "Displays the help message for the given command (and subcommand if given), or displays the list of all commands in the given category."
    )
    + "\n\nCATEGORIES\n\n  "
    + category_list
    + """
...
EXAMPLE
  ...
  """
    + fill(
        'To find all commands related to running and monitoring a job and then display the help message for the command "run", run',
        subsequent_indent="  ",
    )
    + """
...
parser_help.add_argument(
    "command_or_category",
    help=fill(
        "Display the help message for the given command, or the list of all available commands for the given category",
        width_adjustment=-24,
    ),
    nargs="?",
    default=None,
...
parser_help.add_argument(
    "subcommand",
    help=fill(
        "Display the help message for the given subcommand of the command",
        width_adjustment=-23,
    ),
    nargs="?",
    default=None,
...
for category in parser_categories:
    parser_categories[category]["cmds"].append(
        ("help", subparsers._choices_actions[-1].help)
    )
...
def main():
    ...
    if len(args_list) > 0:
        ...
        dxpy.USER_AGENT += " {prog}-{command}".format(
            prog=parser.prog, command=getattr(args, "command", "")
        )
        set_cli_colors(args)
        set_delim(args)
