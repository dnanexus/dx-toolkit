...
void File::init() {
  ...
  //  - Remote file size must match local file size (if file is in "closed" or "closing" state)
  //  - All parts must be in "completed" state, and sum of total size of all parts == local file size (irrespective of file state)
  // If any of these check fail, mark the file as a non-match directly
  assert(out.has("state") && out["state"].type() == JSON_STRING);
  if (out["state"].get<string>() != "open") {
