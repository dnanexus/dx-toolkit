...
| **Python reader** ([`collect_images.py:354`][reader]) | `/.cached_docker_images/{image_name}/{image_name}_{tag}` |
| **Groovy reader** (`DockerImage.groovy:44` in nextaur-app) | `/.cached_docker_images/{image_name}/{image_name}_{tag}` |

[writer]: https://github.com/dnanexus/dx-toolkit/blob/56ccbcef4eff292e5a885ac8bb1574f04e674c07/src/python/dxpy/nextflow/ImageRef.py#L35
[reader]: https://github.com/dnanexus/dx-toolkit/blob/56ccbcef4eff292e5a885ac8bb1574f04e674c07/src/python/dxpy/nextflow/collect_images.py#L354
...
[reject]: https://github.com/dnanexus/dx-toolkit/blob/56ccbcef4eff292e5a885ac8bb1574f04e674c07/src/python/dxpy/nextflow/collect_images.py#L297-L300
...
| Resolve digest for all images | Images missing digest for cache validation | Existing (untagged) → **broadened to all** (PR #1537) |
| Server-side `properties` filter | Reusing cached file with wrong content | **Added** (replaces client-side check) |
| Skip cache when no digest | `digest=None` fallback matching arbitrary files | **Added** |
| `ImageRef`: tag over digest in pull ref | Tagged images pulled by `@sha256:` instead of `:tag` | **Added** |
...
### Mixed digest types in `image_digest` property

The `image_digest` property on cached files stores **different digest types**
depending on how the image was referenced:

| Pipeline reference | Stored `image_digest` | Digest type |
|---|---|---|
| `image:tag` | config digest (from `_resolve_digest`) | Image ID |
| `image` (bare) | config digest (from `_resolve_digest`) | Image ID |
| `image@sha256:abc` | `sha256:abc` (passthrough) | Manifest digest |

This means the same image referenced by tag in one pipeline and by digest in
another will produce separate cached files (different digest types won't match
in the `properties` filter). The duplicate is harmless — extra storage only.

Normalizing to a single digest type would require resolving the config digest
for `@sha256:` refs too, which would invalidate all existing cached files
stored with manifest digests.

### Why `digest_is_original` exists

The `digest_is_original` flag on `_ImageRef` tracks whether the digest came from
the original pipeline reference (`@sha256:...`) or was resolved by `_resolve_digest()`.

This matters for `docker pull`:
- **Original digest** (manifest) — pullable: `docker pull image@sha256:<manifest>`
- **Resolved digest** (config) — NOT pullable: `docker pull image@sha256:<config>` fails

Without this flag, bare image refs like `quay.io/nextflow/bash` would get a resolved
config digest injected into the pull command, causing `docker pull` to fail with
"manifest unknown".
